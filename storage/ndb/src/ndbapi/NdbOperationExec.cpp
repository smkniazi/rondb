...
class OldNdbApiSectionIterator : public GenericSectionIterator {
 ...
 public:
  OldNdbApiSectionIterator(NdbApiSignal *TCREQ, Uint32 dataOffset,
                           Uint32 dataLen, NdbApiSignal *nextSignal)
  ...
  void reset() override { currentPos = firstDataPtr; }
...
void NdbOperation::setRequestInfoTCKEYREQ(bool lastFlag, bool longSignal) {
  ...
  TcKeyReq::setReadCommittedBaseFlag(
      requestInfo,
...
int NdbOperation::doSendKeyReq(int aNodeId, GenericSectionPtr *secs,
                               Uint32 numSecs, bool lastFlag) {
...
Remark:         Sends the TCKEYREQ signal and optional KEYINFO and ATTRINFO
                signals.
******************************************************************************/
int NdbOperation::doSend(int aNodeId, Uint32 lastFlag) {
  ...
  if (m_attribute_record != nullptr) {
    ...
    if (doSendKeyReq(aNodeId, &secs[0], numSecs, lastFlag) == -1) return -1;
  } else {
    /*
     ...
     */
    OldNdbApiSectionIterator keyInfoIter(theTCREQ, inlineKIOffset,
                                         inlineKILength, theTCREQ->next());
    OldNdbApiSectionIterator attrInfoIter(theTCREQ, inlineAIOffset,
                                          inlineAILength, theFirstATTRINFO);
...
int NdbOperation::prepareGetLockHandle() {
...
Remark:         Puts the the data into TCKEYREQ signal and optional KEYINFO and
ATTRINFO signals.
***************************************************************************/
int NdbOperation::prepareSend(Uint32 aTC_ConnectPtr, Uint64 aTransId,
                              AbortOption ao) {
  ...
  if (tInterpretInd != 1) {
    ...
    if ((tOpType == UpdateRequest) || (tOpType == InsertRequest) ||
      ...
      if (tStatus != GetValue) {
        ...
        if (theLockHandle) {
          ...
          if (prepareGetLockHandle() != 0) return -1;
  ...
  if (theTupKeyLen > TcKeyReq::MaxKeyInfo) {
     ...
     */
    if (theLastKEYINFO == nullptr) theLastKEYINFO = theTCREQ->next();

    assert(theLastKEYINFO != nullptr);

    Uint32 lastKeyInfoLen =
        ((theTupKeyLen - TcKeyReq::MaxKeyInfo) % KeyInfo::DataLength);
...
Uint32 NdbOperation::repack_read(Uint32 len) {
  Uint32 i;
  ...
  while (len) {
    ...
    for (i = 0; len && i < AttrInfo::DataLength; i++, len--) {
      AttributeHeader tmp(*ptr++);
      Uint32 id = tmp.getAttributeId();
      if ((id <= prevId) || (id >= NDB_MAX_ATTRIBUTES_IN_TABLE)) {
        // AttrIds not strictly ascending with no duplicates
        // and no pseudo-columns ==  fallback
        return save;
      }
...
Return Value:   Return 0 : successful.
                Return -1: In all other case.
***************************************************************************/
int NdbOperation::prepareSendInterpreted() {
  Uint32 tTotalCurrAI_Len = theTotalCurrAI_Len;
  ...
  if (theStatus == ExecInterpretedValue) {
    ...
    theFinalUpdateSize =
        tTotalCurrAI_Len -
        (tInitReadSize + theInterpretedSize + AttrInfo::SectionSizeInfoLength);

  } else if (theStatus == SubroutineEnd) {
    theSubroutineSize =
        tTotalCurrAI_Len -
        (tInitReadSize + theInterpretedSize + theFinalUpdateSize +
         theFinalReadSize + AttrInfo::SectionSizeInfoLength);
  ...
  */
  while (theFirstBranch != nullptr) {
    ...
    if (tAddress == -1) {
      ...
      // that the application have not programmed the interpreter program
      // correctly.
      //-------------------------------------------------------------------------
  ...
  while (theFirstCall != nullptr) {
    ...
    while (tNdbSubroutine != nullptr) {
      ...
      if (tNdbCall->theSubroutine < tSubroutineCount) {
        // Subroutine Found
        Uint32 tSubroutineAddress =
            tNdbCall->theSubroutine - (tSubroutineCount - 16);
...
*/
int NdbOperation::buildSignalsNdbRecord(Uint32 aTC_ConnectPtr, Uint64 aTransId,
  ...
  if (isScanTakeover) {
    ...
    res = insertKEYINFO_NdbRecord((const char *)keyInfoSrc, keyInfoWords << 2);
    if (res) return res;
  }
  ...
  if (code) {
    ...
    res = insertATTRINFOData_NdbRecord((const char *)sizes, sizeof(sizes));
    if (res) return res;

    /* So that we can go back to set the actual sizes later... */
    attrinfo_section_sizes_ptr =
        (theATTRINFOptr - AttrInfo::SectionSizeInfoLength);
  }
  ...
  if (((m_flags & OF_USE_ANY_VALUE) != 0) && (tOpType == DeleteRequest)) {
  ...
  if ((tOpType == InsertRequest) || (tOpType == WriteRequest) ||
      (tOpType == UpdateRequest) || (tOpType == RefreshRequest)) {
    ...
    if (m_extraSetValues != nullptr) {
      for (Uint32 i = 0; i < m_numExtraSetValues; i++) {
        ...
        if (pvalue == nullptr)
          ...
          if (extraCol->getArrayType() !=
              NdbDictionary::Column::ArrayTypeFixed) {
            Uint32 lengthInfoBytes;
            if (!NdbSqlUtil::get_var_length((Uint32)extraCol->getType(), pvalue,
                                            length, lengthInfoBytes, length)) {
...
*/
int NdbOperation::prepareSendNdbRecord(AbortOption ao) {
...
*/
Uint32 NdbOperation::fillTcKeyReqHdr(TcKeyReq *tcKeyReq, Uint32 connectPtr,
                                     Uint64 transId) {
  Uint32 hdrLen;
  UintR *hdrPtr;
...
/*
  ...
  Return 0 on success, -1 on error.
*/
int NdbOperation::allocAttrInfo() {
  NdbApiSignal *tSignal;
...
int NdbOperation::insertKEYINFO_NdbRecord(const char *value, Uint32 byteSize) {
...
int NdbOperation::insertATTRINFOHdr_NdbRecord(Uint32 attrId, Uint32 attrLen) {
...
int NdbOperation::checkState_TransId(const NdbApiSignal *aSignal) {
...
Return Value:   Return 0 : send was successful.
                Return -1: In all other case.
Parameters:     aSignal: the signal object that contains the TCKEYREF signal

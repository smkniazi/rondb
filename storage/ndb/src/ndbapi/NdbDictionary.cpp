...
NdbDictionary::RecordType NdbDictionary::getRecordType(
    const NdbRecord *record) {
  return NdbDictionaryImpl::getRecordType(record);
}

const char *NdbDictionary::getRecordTableName(const NdbRecord *record) {
  return NdbDictionaryImpl::getRecordTableName(record);
}

const char *NdbDictionary::getRecordIndexName(const NdbRecord *record) {
  return NdbDictionaryImpl::getRecordIndexName(record);
}

bool NdbDictionary::getFirstAttrId(const NdbRecord *record,
                                   Uint32 &firstAttrId) {
  return NdbDictionaryImpl::getNextAttrIdFrom(record, 0, firstAttrId);
}

bool NdbDictionary::getNextAttrId(const NdbRecord *record, Uint32 &attrId) {
  return NdbDictionaryImpl::getNextAttrIdFrom(record, attrId + 1, attrId);
}

bool NdbDictionary::getOffset(const NdbRecord *record, Uint32 attrId,
                              Uint32 &offset) {
  return NdbDictionaryImpl::getOffset(record, attrId, offset);
}

bool NdbDictionary::getNullBitOffset(const NdbRecord *record, Uint32 attrId,
                                     Uint32 &nullbit_byte_offset,
                                     Uint32 &nullbit_bit_in_byte) {
  return NdbDictionaryImpl::getNullBitOffset(
      record, attrId, nullbit_byte_offset, nullbit_bit_in_byte);
}

const char *NdbDictionary::getValuePtr(const NdbRecord *record, const char *row,
                                       Uint32 attrId) {
  return NdbDictionaryImpl::getValuePtr(record, row, attrId);
}

char *NdbDictionary::getValuePtr(const NdbRecord *record, char *row,
                                 Uint32 attrId) {
  return NdbDictionaryImpl::getValuePtr(record, row, attrId);
}

bool NdbDictionary::isNull(const NdbRecord *record, const char *row,
                           Uint32 attrId) {
  return NdbDictionaryImpl::isNull(record, row, attrId);
}

int NdbDictionary::setNull(const NdbRecord *record, char *row, Uint32 attrId,
                           bool value) {
  return NdbDictionaryImpl::setNull(record, row, attrId, value);
}

Uint32 NdbDictionary::getRecordRowLength(const NdbRecord *record) {
  return NdbDictionaryImpl::getRecordRowLength(record);
}

const unsigned char *NdbDictionary::getEmptyBitmask() {
...
NdbDictionary::Object::Status NdbDictionary::ObjectId::getObjectStatus() const {
  return m_impl.m_status;
}

int NdbDictionary::ObjectId::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::ObjectId::getObjectId() const { return m_impl.m_id; }
...
NdbDictionary::Column::Column(NdbColumnImpl &impl) : m_impl(impl) {}
...
NdbDictionary::Column &NdbDictionary::Column::operator=(
...
void NdbDictionary::Column::setType(Type t) { m_impl.init(t); }

NdbDictionary::Column::Type NdbDictionary::Column::getType() const {
  return m_impl.m_type;
}

void NdbDictionary::Column::setPrecision(int val) { m_impl.m_precision = val; }

int NdbDictionary::Column::getPrecision() const { return m_impl.m_precision; }

void NdbDictionary::Column::setScale(int val) { m_impl.m_scale = val; }

int NdbDictionary::Column::getScale() const { return m_impl.m_scale; }

void NdbDictionary::Column::setLength(int length) { m_impl.m_length = length; }

int NdbDictionary::Column::getLength() const { return m_impl.m_length; }

void NdbDictionary::Column::setInlineSize(int size) {
  m_impl.m_precision = size;
}

void NdbDictionary::Column::setCharset(CHARSET_INFO *cs) { m_impl.m_cs = cs; }

CHARSET_INFO *NdbDictionary::Column::getCharset() const { return m_impl.m_cs; }

int NdbDictionary::Column::getCharsetNumber() const {
  return m_impl.m_cs->number;
}

int NdbDictionary::Column::getInlineSize() const { return m_impl.m_precision; }

void NdbDictionary::Column::setPartSize(int size) { m_impl.m_scale = size; }

int NdbDictionary::Column::getPartSize() const { return m_impl.m_scale; }

void NdbDictionary::Column::setStripeSize(int size) { m_impl.m_length = size; }

int NdbDictionary::Column::getStripeSize() const { return m_impl.m_length; }

int NdbDictionary::Column::getSize() const { return m_impl.m_attrSize; }

void NdbDictionary::Column::setNullable(bool val) { m_impl.m_nullable = val; }

bool NdbDictionary::Column::getNullable() const { return m_impl.m_nullable; }

void NdbDictionary::Column::setPrimaryKey(bool val) { m_impl.m_pk = val; }

bool NdbDictionary::Column::getPrimaryKey() const { return m_impl.m_pk; }

void NdbDictionary::Column::setPartitionKey(bool val) {
...
const NdbDictionary::Table *NdbDictionary::Column::getBlobTable() const {
  ...
  if (t) return t->m_facade;
  return nullptr;
}

void NdbDictionary::Column::setAutoIncrement(bool val) {
...
void NdbDictionary::Column::setAutoIncrementInitialValue(Uint64 val) {
  m_impl.m_autoIncrementInitialValue = val;
}

int NdbDictionary::Column::getSizeInBytesForRecord() const {
 ...
 */
int NdbDictionary::Column::setDefaultValue(const char *) { return -1; }
...
*/
int NdbDictionary::Column::setDefaultValue(const void *defaultValue,
                                           unsigned int n) {
  if (defaultValue == nullptr) return m_impl.m_defaultValue.assign(nullptr, 0);

  return m_impl.m_defaultValue.assign(defaultValue, n);
}

const void *NdbDictionary::Column::getDefaultValue(unsigned int *len) const {
  if (len) *len = m_impl.m_defaultValue.length();

  return m_impl.m_defaultValue.get_data();
}

int NdbDictionary::Column::getColumnNo() const { return m_impl.m_column_no; }

int NdbDictionary::Column::getAttrId() const { return m_impl.m_attrId; }

bool NdbDictionary::Column::equal(const NdbDictionary::Column &col) const {
...
int NdbDictionary::Column::getBlobVersion() const {
  return m_impl.getBlobVersion();
}

void NdbDictionary::Column::setBlobVersion(int blobVersion) {
  m_impl.setBlobVersion(blobVersion);
}

void NdbDictionary::Column::setDynamic(bool val) { m_impl.m_dynamic = val; }

bool NdbDictionary::Column::getDynamic() const { return m_impl.m_dynamic; }

bool NdbDictionary::Column::getIndexSourced() const {
  return m_impl.m_indexSourced;
}

int NdbDictionary::Column::isBindable(const NdbDictionary::Column &col) const {
...
NdbDictionary::Table::Table(NdbTableImpl &impl) : m_impl(impl) {}
...
NdbDictionary::Table &NdbDictionary::Table::operator=(
...
const char *NdbDictionary::Table::getName() const { return m_impl.getName(); }
...
NdbDictionary::Column *NdbDictionary::Table::getColumn(const int attrId) {
  return m_impl.getColumn(attrId);
}

void NdbDictionary::Table::setLogging(bool val) { m_impl.m_logging = val; }

bool NdbDictionary::Table::getLogging() const { return m_impl.m_logging; }

void NdbDictionary::Table::setFragmentType(FragmentType ft) {
  m_impl.m_fragmentType = ft;
}

NdbDictionary::Object::FragmentType NdbDictionary::Table::getFragmentType()
    const {
  return m_impl.m_fragmentType;
}

void NdbDictionary::Table::setKValue(int kValue) { m_impl.m_kvalue = kValue; }

int NdbDictionary::Table::getKValue() const { return m_impl.m_kvalue; }
...
Uint64 NdbDictionary::Table::getMaxRows() const { return m_impl.m_max_rows; }

void NdbDictionary::Table::setMinRows(Uint64 minRows) {
  m_impl.m_min_rows = minRows;
}

Uint64 NdbDictionary::Table::getMinRows() const { return m_impl.m_min_rows; }

void NdbDictionary::Table::setDefaultNoPartitionsFlag(Uint32 flag) {
...
const char *NdbDictionary::Table::getPrimaryKey(int no) const {
  ...
  for (unsigned i = 0; i < m_impl.m_columns.size(); i++) {
    if (m_impl.m_columns[i]->m_pk) {
      if (count++ == no) return m_impl.m_columns[i]->m_name.c_str();
    }
  }
  return nullptr;
}
...
void NdbDictionary::Table::setSingleUserMode(
    enum NdbDictionary::Table::SingleUserMode mode) {
...
void NdbDictionary::Table::setPartitionBalance(
    NdbDictionary::Object::PartitionBalance arg) {
...
const char *NdbDictionary::Table::getPartitionBalanceString() const {
...
NdbDictionary::Table::getPartitionBalance(const char str[]) {
  ...
  return NdbDictionary::Object::PartitionBalance(
      0); /* No matching partition balance */
}

const char *NdbDictionary::Table::getPartitionBalanceString(
...
int NdbDictionary::Table::setFrm(const void *data, Uint32 len) {
  return m_impl.setFrm(data, len);
}

int NdbDictionary::Table::setExtraMetadata(Uint32 version, const void *data,
                                           Uint32 data_length) {
  return m_impl.setExtraMetadata(version, data, data_length);
}

int NdbDictionary::Table::getExtraMetadata(Uint32 &version, void **data,
...
int NdbDictionary::Table::setFragmentData(const Uint32 *data, Uint32 cnt) {
...
int NdbDictionary::Table::setRangeListData(const Int32 *data, Uint32 len) {
...
NdbDictionary::Object::Status NdbDictionary::Table::getObjectStatus() const {
  return m_impl.m_status;
}
...
int NdbDictionary::Table::getObjectVersion() const { return m_impl.m_version; }

int NdbDictionary::Table::getObjectId() const { return m_impl.m_id; }

bool NdbDictionary::Table::equal(const NdbDictionary::Table &col) const {
  return m_impl.equal(col.m_impl);
}
...
bool NdbDictionary::Table::getTemporary() const { return m_impl.m_temporary; }

void NdbDictionary::Table::setTemporary(bool val) { m_impl.m_temporary = val; }

int NdbDictionary::Table::createTableInDb(Ndb *pNdb, bool) const {
  const NdbDictionary::Table *pTab = pNdb->getDictionary()->getTable(getName());
  if (pTab != nullptr && equal(*pTab)) return 0;
  if (pTab != nullptr && !equal(*pTab)) return -1;
  return pNdb->getDictionary()->createTable(*this);
}

bool NdbDictionary::Table::getTablespace(Uint32 *id, Uint32 *version) const {
  if (m_impl.m_tablespace_id == RNIL) return false;
  if (id) *id = m_impl.m_tablespace_id;
  if (version) *version = m_impl.m_version;
  return true;
}
...
int NdbDictionary::Table::setTablespace(const NdbDictionary::Tablespace &ts) {
...
bool NdbDictionary::Table::getHashMap(Uint32 *id, Uint32 *version) const {
  if (m_impl.m_hash_map_id == RNIL) return false;
  if (id) *id = m_impl.m_hash_map_id;
  if (version) *version = m_impl.m_hash_map_version;
  return true;
}

int NdbDictionary::Table::setHashMap(const NdbDictionary::HashMap &hm) {
  m_impl.m_hash_map_id = hm.getObjectId();
  m_impl.m_hash_map_version = hm.getObjectVersion();
  return 0;
}

void NdbDictionary::Table::setRowChecksumIndicator(bool val) {
...
void NdbDictionary::Table::setRowGCIIndicator(bool val) {
...
Uint32 NdbDictionary::Table::getExtraRowGciBits() const {
...
Uint32 NdbDictionary::Table::getExtraRowAuthorBits() const {
  return m_impl.m_extra_row_author_bits;
}

void NdbDictionary::Table::setReadBackupFlag(bool val) {
  m_impl.m_read_backup = val;
}

bool NdbDictionary::Table::getReadBackupFlag() const {
  return m_impl.m_read_backup;
}

void NdbDictionary::Table::setForceVarPart(bool val) {
...
bool NdbDictionary::Table::hasDefaultValues() const {
...
int NdbDictionary::Table::aggregate(NdbError &) { return m_impl.aggregate(); }

int NdbDictionary::Table::validate(NdbError &) { return m_impl.validate(); }
...
void NdbDictionary::Table::assignObjId(const NdbDictionary::ObjectId &_objId) {
...
void NdbDictionary::Table::setStorageType(
    NdbDictionary::Column::StorageType type) {
  m_impl.m_storageType = type;
}

NdbDictionary::Column::StorageType NdbDictionary::Table::getStorageType()
    const {
  return (NdbDictionary::Column::StorageType)m_impl.m_storageType;
}

void NdbDictionary::Table::setFullyReplicated(bool val) {
  m_impl.m_fully_replicated = val;
}

bool NdbDictionary::Table::getFullyReplicated() const {
...
Uint32 NdbDictionary::Table::getRowChecksum() { return m_impl.m_row_checksum; }
...
NdbDictionary::Index::Index(NdbIndexImpl &impl) : m_impl(impl) {}
...
const char *NdbDictionary::Index::getName() const { return m_impl.getName(); }

int NdbDictionary::Index::setTable(const char *table) {
...
const NdbDictionary::Column *NdbDictionary::Index::getColumn(
    unsigned no) const {
  if (no < m_impl.m_columns.size()) return m_impl.m_columns[no];
  return nullptr;
}
...
int NdbDictionary::Index::addIndexColumns(int noOfNames, const char **names) {
...
void NdbDictionary::Index::setType(NdbDictionary::Index::Type t) {
...
void NdbDictionary::Index::setLogging(bool val) { m_impl.m_logging = val; }

bool NdbDictionary::Index::getTemporary() const { return m_impl.m_temporary; }

void NdbDictionary::Index::setTemporary(bool val) { m_impl.m_temporary = val; }

bool NdbDictionary::Index::getLogging() const { return m_impl.m_logging; }

NdbDictionary::Object::Status NdbDictionary::Index::getObjectStatus() const {
  return m_impl.m_table->m_status;
}

int NdbDictionary::Index::getObjectVersion() const {
  return m_impl.m_table->m_version;
}

int NdbDictionary::Index::getObjectId() const { return m_impl.m_table->m_id; }
...
NdbDictionary::OptimizeTableHandle::OptimizeTableHandle(
    NdbOptimizeTableHandleImpl &impl)
    : m_impl(impl) {}
...
int NdbDictionary::OptimizeTableHandle::next() { return m_impl.next(); }
...
NdbDictionary::OptimizeIndexHandle::OptimizeIndexHandle(
    NdbOptimizeIndexHandleImpl &impl)
    : m_impl(impl) {}
...
int NdbDictionary::OptimizeIndexHandle::next() { return m_impl.next(); }

int NdbDictionary::OptimizeIndexHandle::close() {
  int result = m_impl.close();
  return result;
}
...
NdbDictionary::Event::Event(NdbEventImpl &impl) : m_impl(impl) {}
...
const char *NdbDictionary::Event::getName() const { return m_impl.getName(); }

void NdbDictionary::Event::setTable(const Table &table) {
  m_impl.setTable(table);
}

int NdbDictionary::Event::setTable(const Table *table) {
...
int NdbDictionary::Event::setTable(const char *table) {
  return m_impl.setTable(table);
}
...
void NdbDictionary::Event::addTableEvent(const TableEvent t) {
  m_impl.addTableEvent(t);
}

bool NdbDictionary::Event::getTableEvent(const TableEvent t) const {
...
NdbDictionary::Event::EventDurability NdbDictionary::Event::getDurability()
    const {
  return ED_PERMANENT;
}

void NdbDictionary::Event::setReport(EventReport r) { m_impl.setReport(r); }

void NdbDictionary::Event::setReportOptions(Uint32 report_options) {
...
Uint32 NdbDictionary::Event::getReportOptions() const {
...
void NdbDictionary::Event::addEventColumn(const char *name) {
  const Column c(name);
  addColumn(c);
}

void NdbDictionary::Event::addEventColumns(int n, const char **names) {
  for (int i = 0; i < n; i++) addEventColumn(names[i]);
}
...
const NdbDictionary::Column *NdbDictionary::Event::getEventColumn(
    unsigned no) const {
...
int NdbDictionary::Event::getObjectVersion() const { return m_impl.m_version; }

int NdbDictionary::Event::getObjectId() const { return m_impl.m_id; }

void NdbDictionary::Event::print() { m_impl.print(); }
...
NdbDictionary::Tablespace::Tablespace(NdbTablespaceImpl &impl) : m_impl(impl) {}
...
void NdbDictionary::Tablespace::setName(const char *name) {
  m_impl.m_name.assign(name);
}

const char *NdbDictionary::Tablespace::getName() const {
  return m_impl.m_name.c_str();
}

void NdbDictionary::Tablespace::setAutoGrowSpecification(
...
void NdbDictionary::Tablespace::setExtentSize(Uint32 sz) {
  m_impl.m_extent_size = sz;
}

Uint32 NdbDictionary::Tablespace::getExtentSize() const {
  return m_impl.m_extent_size;
}

void NdbDictionary::Tablespace::setDefaultLogfileGroup(const char *name) {
  m_impl.m_logfile_group_id = ~0;
  m_impl.m_logfile_group_version = ~0;
  m_impl.m_logfile_group_name.assign(name);
}

void NdbDictionary::Tablespace::setDefaultLogfileGroup(
...
const char *NdbDictionary::Tablespace::getDefaultLogfileGroup() const {
  return m_impl.m_logfile_group_name.c_str();
}

Uint32 NdbDictionary::Tablespace::getDefaultLogfileGroupId() const {
  return m_impl.m_logfile_group_id;
}

NdbDictionary::Object::Status NdbDictionary::Tablespace::getObjectStatus()
    const {
  return m_impl.m_status;
}

int NdbDictionary::Tablespace::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::Tablespace::getObjectId() const { return m_impl.m_id; }
...
NdbDictionary::LogfileGroup::LogfileGroup(
    const NdbDictionary::LogfileGroup &org)
...
void NdbDictionary::LogfileGroup::setAutoGrowSpecification(
    const NdbDictionary::AutoGrowSpecification &spec) {
  m_impl.m_grow_spec = spec;
}
const NdbDictionary::AutoGrowSpecification &
...
NdbDictionary::Object::Status NdbDictionary::LogfileGroup::getObjectStatus()
    const {
  return m_impl.m_status;
}

int NdbDictionary::LogfileGroup::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::LogfileGroup::getObjectId() const { return m_impl.m_id; }

/*****************************************************************
 * Datafile facade
 */
NdbDictionary::Datafile::Datafile() : m_impl(*new NdbDatafileImpl(*this)) {}

NdbDictionary::Datafile::Datafile(NdbDatafileImpl &impl) : m_impl(impl) {}
...
void NdbDictionary::Datafile::setPath(const char *path) {
...
void NdbDictionary::Datafile::setSize(Uint64 sz) { m_impl.m_size = sz; }

Uint64 NdbDictionary::Datafile::getSize() const { return m_impl.m_size; }

Uint64 NdbDictionary::Datafile::getFree() const { return m_impl.m_free; }

int NdbDictionary::Datafile::setTablespace(const char *tablespace) {
  m_impl.m_filegroup_id = ~0;
  m_impl.m_filegroup_version = ~0;
  return !m_impl.m_filegroup_name.assign(tablespace);
}

int NdbDictionary::Datafile::setTablespace(
    const NdbDictionary::Tablespace &ts) {
...
void NdbDictionary::Datafile::getTablespaceId(
    NdbDictionary::ObjectId *dst) const {
  if (dst) {
...
NdbDictionary::Object::Status NdbDictionary::Datafile::getObjectStatus() const {
  return m_impl.m_status;
}

int NdbDictionary::Datafile::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::Datafile::getObjectId() const { return m_impl.m_id; }

/*****************************************************************
 * Undofile facade
 */
NdbDictionary::Undofile::Undofile() : m_impl(*new NdbUndofileImpl(*this)) {}

NdbDictionary::Undofile::Undofile(NdbUndofileImpl &impl) : m_impl(impl) {}
...
void NdbDictionary::Undofile::setSize(Uint64 sz) { m_impl.m_size = sz; }

Uint64 NdbDictionary::Undofile::getSize() const { return m_impl.m_size; }

void NdbDictionary::Undofile::setLogfileGroup(const char *logfileGroup) {
  m_impl.m_filegroup_id = ~0;
  m_impl.m_filegroup_version = ~0;
  m_impl.m_filegroup_name.assign(logfileGroup);
}

void NdbDictionary::Undofile::setLogfileGroup(
...
void NdbDictionary::Undofile::getLogfileGroupId(
    NdbDictionary::ObjectId *dst) const {
  if (dst) {
    NdbDictObjectImpl::getImpl(*dst).m_id = m_impl.m_filegroup_id;
    NdbDictObjectImpl::getImpl(*dst).m_version = m_impl.m_filegroup_version;
...
int NdbDictionary::Undofile::getObjectId() const { return m_impl.m_id; }

/*****************************************************************
 * HashMap facade
 */
NdbDictionary::HashMap::HashMap() : m_impl(*new NdbHashMapImpl(*this)) {}

NdbDictionary::HashMap::HashMap(NdbHashMapImpl &impl) : m_impl(impl) {}
...
void NdbDictionary::HashMap::setName(const char *path) {
...
void NdbDictionary::HashMap::setMap(const Uint32 *map, Uint32 len) {
  m_impl.m_map.assign(map, len);
}

Uint32 NdbDictionary::HashMap::getMapLen() const { return m_impl.m_map.size(); }

int NdbDictionary::HashMap::getMapValues(Uint32 *dst, Uint32 len) const {
  if (len != getMapLen()) return -1;

  memcpy(dst, m_impl.m_map.getBase(), sizeof(Uint32) * len);
  return 0;
}

bool NdbDictionary::HashMap::equal(const NdbDictionary::HashMap &obj) const {
  return m_impl.m_map.equal(obj.m_impl.m_map);
}

NdbDictionary::Object::Status NdbDictionary::HashMap::getObjectStatus() const {
  return m_impl.m_status;
}

int NdbDictionary::HashMap::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::HashMap::getObjectId() const { return m_impl.m_id; }
...
int NdbDictionary::Dictionary::getDefaultHashMap(NdbDictionary::HashMap &dst,
                                             Uint32 buckets,
                                                 Uint32 partitionCount) {
  BaseString tmp;
  tmp.assfmt("DEFAULT-HASHMAP-%u-%u", buckets, partitionCount);
...
int NdbDictionary::Dictionary::getHashMap(NdbDictionary::HashMap &dst,
  ...
  return m_impl.m_receiver.get_hashmap(
      NdbHashMapImpl::getImpl(dst), NdbTableImpl::getImpl(*tab).m_hash_map_id);
}

int NdbDictionary::Dictionary::initDefaultHashMap(NdbDictionary::HashMap &dst,
                                                  Uint32 partitionCount) {
  return initDefaultHashMap(dst, m_impl.getDefaultHashmapSize(),
                            partitionCount);
}
...
int NdbDictionary::Dictionary::prepareHashMap(const Table &oldTableF,
                                              Table &newTableF) {
...
int NdbDictionary::Dictionary::prepareHashMap(const Table &oldTableF,
  ...
  if (oldTable.getFragmentType() == NdbDictionary::Object::HashMapPartition) {
    ...
    Uint32 newcnt;
    if (newTable.getPartitionBalance() ==
        NdbDictionary::Table::PartitionBalance_Specific) {
    ...
    if (newcnt == 0) {
      ...
      int ret = m_impl.m_receiver.create_hashmap(
          NdbHashMapImpl::getImpl(newmapF), &NdbDictObjectImpl::getImpl(tmp),
          flags, newTable.getPartitionBalance());
      if (ret) {
        return -1;
      }

      HashMap hm;
      ret = m_impl.m_receiver.get_hashmap(NdbHashMapImpl::getImpl(hm),
                                          tmp.getObjectId());
      if (ret) {
        return -1;
      }
      ...
      for (Uint32 i = 0; i < hm.getMapLen(); i++) {
        if (values[i] > newcnt) newcnt = values[i];
      }
      ...
      if (!newTable.getFullyReplicated()) {
        ...
        /**
         * For fully replicated table new fragment count is still unknown.  Keep
         * it zero.
         */
  ...
  retry:
    if (cnt == 0) {
      newmap.m_name.assfmt("HASHMAP-%u-%u-%u", newmapsize, oldcnt, newcnt);
    } else {
      newmap.m_name.assfmt("HASHMAP-%u-%u-%u-#%u", newmapsize, oldcnt, newcnt,
                           cnt);
    }
...
NdbDictionary::ForeignKey::ForeignKey(NdbForeignKeyImpl &impl) : m_impl(impl) {}
...
const char *NdbDictionary::ForeignKey::getName() const {
  return m_impl.m_name.c_str();
}

NdbDictionary::Object::Status NdbDictionary::ForeignKey::getObjectStatus()
    const {
  return m_impl.m_status;
}

int NdbDictionary::ForeignKey::getObjectVersion() const {
  return m_impl.m_version;
}

int NdbDictionary::ForeignKey::getObjectId() const { return m_impl.m_id; }

void NdbDictionary::ForeignKey::setName(const char *name) {
  m_impl.m_name.assign(name);
}

void NdbDictionary::ForeignKey::setParent(const Table &tab, const Index *idx,
                                          const Column *cols[]) {
  ...
  if (cols) {
    for (Uint32 i = 0; cols[i] != nullptr; i++) {
...
const char *NdbDictionary::ForeignKey::getParentTable() const {
  return m_impl.m_references[0].m_name.c_str();
}

const char *NdbDictionary::ForeignKey::getParentIndex() const {
  if (m_impl.m_references[2].m_name.empty()) return nullptr;
  return m_impl.m_references[2].m_name.c_str();
}

void NdbDictionary::ForeignKey::setChild(const Table &tab, const Index *idx,
                                         const Column *cols[]) {
...
const char *NdbDictionary::ForeignKey::getChildTable() const {
  return m_impl.m_references[1].m_name.c_str();
}

const char *NdbDictionary::ForeignKey::getChildIndex() const {
  if (m_impl.m_references[3].m_name.empty()) return nullptr;
...
void NdbDictionary::ForeignKey::setOnUpdateAction(FkAction _action) {
...
void NdbDictionary::ForeignKey::setOnDeleteAction(FkAction _action) {
  m_impl.m_on_delete_action = _action;
}

unsigned NdbDictionary::ForeignKey::getParentColumnCount() const {
  return m_impl.m_parent_columns.size();
}

int NdbDictionary::ForeignKey::getParentColumnNo(unsigned no) const {
...
unsigned NdbDictionary::ForeignKey::getChildColumnCount() const {
  return m_impl.m_child_columns.size();
}

int NdbDictionary::ForeignKey::getChildColumnNo(unsigned no) const {
...
NdbDictionary::Dictionary::Dictionary(NdbDictionaryImpl &impl) : m_impl(impl) {}
...
#define DO_TRANS(ret, action) \
  { \
    ...
    if ((trans || (ret = beginSchemaTrans()) == 0) && (ret = (action)) == 0 && \
...
int NdbDictionary::Dictionary::createTable(const Table &t) {
  return createTable(t, nullptr);
}

int NdbDictionary::Dictionary::createTable(const Table &t, ObjectId *objId) {
  int ret;
  ObjectId tmp;
  if (objId == nullptr) objId = &tmp;

  if (likely(!is_ndb_blob_table(t.getName()))) {
    DO_TRANS(ret, m_impl.createTable(NdbTableImpl::getImpl(t),
...
int NdbDictionary::Dictionary::optimizeIndex(
    const Index &ind, NdbDictionary::OptimizeIndexHandle &h) {
...
int NdbDictionary::Dictionary::dropTable(Table &t) {
  int ret;
  if (likely(!is_ndb_blob_table(t.getName()))) {
    DO_TRANS(ret, m_impl.dropTable(NdbTableImpl::getImpl(t)));
  } else {
    /* 4249 : Invalid table */
...
int NdbDictionary::Dictionary::dropTableGlobal(const Table &t) {
  return dropTableGlobal(t, 0);
}

int NdbDictionary::Dictionary::dropTableGlobal(const Table &t, int flags) {
  int ret;
  if (likely(!is_ndb_blob_table(t.getName()))) {
    DO_TRANS(ret, m_impl.dropTableGlobal(NdbTableImpl::getImpl(t), flags));
  } else {
    /* 4249 : Invalid table */
    m_impl.m_error.code = 4249;
    ret = -1;
...
int NdbDictionary::Dictionary::dropTable(const char *name) {
  ...
  if (likely(!is_ndb_blob_table(name))) {
    DO_TRANS(ret, m_impl.dropTable(name));
  } else {
    /* 4307 : Invalid table name */
    m_impl.m_error.code = 4307;
    ret = -1;
...
int NdbDictionary::Dictionary::alterTable(const Table &f, const Table &t) {
  int ret;
  DO_TRANS(ret, m_impl.alterTable(NdbTableImpl::getImpl(f),
                                  NdbTableImpl::getImpl(t)));
  return ret;
}

int NdbDictionary::Dictionary::alterTableGlobal(const Table &f,
                                                const Table &t) {
  int ret;
  DO_TRANS(ret, m_impl.alterTableGlobal(NdbTableImpl::getImpl(f),
                                        NdbTableImpl::getImpl(t)));
  return ret;
}

const NdbDictionary::Table *NdbDictionary::Dictionary::getTable(
    const char *name, void **data) const {
  NdbTableImpl *t = m_impl.getTable(name, data);
  if (t) return t->m_facade;
  return nullptr;
}

const NdbDictionary::Index *NdbDictionary::Dictionary::getIndexGlobal(
    const char *indexName, const Table &ndbtab) const {
  NdbIndexImpl *i =
      m_impl.getIndexGlobal(indexName, NdbTableImpl::getImpl(ndbtab));
  if (i) return i->m_facade;
  return nullptr;
}

const NdbDictionary::Index *NdbDictionary::Dictionary::getIndexGlobal(
    const char *indexName, const char *tableName) const {
  NdbIndexImpl *i = m_impl.getIndexGlobal(indexName, tableName);
  if (i) return i->m_facade;
  return nullptr;
}
...
NdbRecord *NdbDictionary::Dictionary::createRecord(
    const Table *table, const RecordSpecification *recSpec, Uint32 length,
    Uint32 elemSize, Uint32 flags) {
  ...
  if (globalTab == nullptr) /* An error is set on the dictionary */
    return nullptr;
  ...
  NdbRecord *result =
      m_impl.createRecord(globalTabImpl, recSpec, length, elemSize, flags,
...
NdbRecord *NdbDictionary::Dictionary::createRecord(
    const Index *index, const Table *table, const RecordSpecification *recSpec,
    Uint32 length, Uint32 elemSize, Uint32 flags) {
  ...
  myNdb->setDatabaseName(
      Ndb::getDatabaseFromInternalName(tabImpl->m_internalName.c_str())
          .c_str());
  myNdb->setDatabaseSchemaName(
  ...
  if (globalIndex == nullptr) /* An error is set on the dictionary */
    return nullptr;
  ...
  NdbRecord *result = m_impl.createRecord(globalIndexImpl->m_table, recSpec,
                                          length, elemSize, flags,
                                         false); // Not default NdbRecord
...
NdbRecord *NdbDictionary::Dictionary::createRecord(
    const Index *index, const RecordSpecification *recSpec, Uint32 length,
    Uint32 elemSize, Uint32 flags) {
  const NdbDictionary::Table *table = getTable(index->getTable());
  if (!table) return nullptr;
  return createRecord(index, table, recSpec, length, elemSize, flags);
}
...
const NdbDictionary::Table *NdbDictionary::Dictionary::getBlobTable(
    const NdbDictionary::Table *table, const char *col_name) {
...
const NdbDictionary::Table *NdbDictionary::Dictionary::getBlobTable(
    const NdbDictionary::Table *table, Uint32 col_no) {
...
void NdbDictionary::Dictionary::invalidateTable(const char *name) {
  ...
  if (t) m_impl.invalidateObject(*t);
  DBUG_VOID_RETURN;
}
...
void NdbDictionary::Dictionary::removeCachedTable(const char *name) {
  NdbTableImpl *t = m_impl.getTable(name);
  if (t) m_impl.removeCachedObject(*t);
}
...
int NdbDictionary::Dictionary::createIndex(const Index &ind, bool offline) {
  int ret;
  DO_TRANS(ret, m_impl.createIndex(NdbIndexImpl::getImpl(ind), offline));
  return ret;
}

int NdbDictionary::Dictionary::createIndex(const Index &ind, const Table &tab,
                                           bool offline) {
  int ret;
  DO_TRANS(ret, m_impl.createIndex(NdbIndexImpl::getImpl(ind),
                                   NdbTableImpl::getImpl(tab), offline));
  return ret;
}
...
int NdbDictionary::Dictionary::dropIndex(const char *indexName,
  ...
  int ret;
  DO_TRANS(ret, m_impl.dropIndex(indexName, tableName, false));
  return ret;
}

int NdbDictionary::Dictionary::dropIndexGlobal(const Index &ind) {
  int ret;
  DO_TRANS(ret, m_impl.dropIndexGlobal(NdbIndexImpl::getImpl(ind), false));
  return ret;
}

int NdbDictionary::Dictionary::updateIndexStat(const Index &index,
                                               const Table &table) {
  int ret;
  DO_TRANS(ret, m_impl.updateIndexStat(NdbIndexImpl::getImpl(index),
                                       NdbTableImpl::getImpl(table)));
  return ret;
}

int NdbDictionary::Dictionary::updateIndexStat(Uint32 indexId,
                                           Uint32 indexVersion,
                                               Uint32 tableId) {
  int ret;
  DO_TRANS(ret, m_impl.updateIndexStat(indexId, indexVersion, tableId));
  return ret;
}

int NdbDictionary::Dictionary::deleteIndexStat(const Index &index,
                                               const Table &table) {
  int ret;
  DO_TRANS(ret, m_impl.deleteIndexStat(NdbIndexImpl::getImpl(index),
                                       NdbTableImpl::getImpl(table)));
  return ret;
}

int NdbDictionary::Dictionary::deleteIndexStat(Uint32 indexId,
                                           Uint32 indexVersion,
                                               Uint32 tableId) {
  int ret;
  DO_TRANS(ret, m_impl.deleteIndexStat(indexId, indexVersion, tableId));
  return ret;
}

const NdbDictionary::Index *NdbDictionary::Dictionary::getIndex(
    const char *indexName, const char *tableName) const {
...
const NdbDictionary::Index *NdbDictionary::Dictionary::getIndex(
    const char *indexName, const NdbDictionary::Table &base) const {
...
int NdbDictionary::Dictionary::forceGCPWait() { return forceGCPWait(0); }

int NdbDictionary::Dictionary::forceGCPWait(int type) {
...
void NdbDictionary::Dictionary::removeCachedIndex(const char *indexName,
                                                  const char *tableName) {
  NdbIndexImpl *i = m_impl.getIndex(indexName, tableName);
  if (i) {
    assert(i->m_table != nullptr);
...
int NdbDictionary::Dictionary::createEvent(const Event &ev) {
  return m_impl.createEvent(NdbEventImpl::getImpl(ev));
}

int NdbDictionary::Dictionary::dropEvent(const char *eventName, int force) {
  return m_impl.dropEvent(eventName, force);
}

const NdbDictionary::Event *NdbDictionary::Dictionary::getEvent(
...
int NdbDictionary::Dictionary::listObjects(List &list,
  ...
  return listObjects(list, type, true /* fully qualified */);
}
...
int NdbDictionary::Dictionary::listIndexes(
    List &list, const NdbDictionary::Table &table) const {
  return m_impl.listIndexes(list, table.getTableId(),
                            true /* fully qualified */);
}

int NdbDictionary::Dictionary::listDependentObjects(
    List &list, const NdbDictionary::Table &tab) const {
  return m_impl.listDependentObjects(list, tab.getTableId());
}

const struct NdbError &NdbDictionary::Dictionary::getNdbError() const {
  return m_impl.getNdbError();
}

int NdbDictionary::Dictionary::getWarningFlags() const { return m_impl.m_warn; }

// printers

static void pretty_print_string(NdbOut &out,
  ...
  if (is_binary && f.hex_format) {
    ...
    out.print("0x");
    for (len = 0; len < (int)sz; len++) out.print("%02X", (int)ref[len]);
    return;
  }
  ...
  for (len = 0; len < (int)sz && ref[i] != 0; len++) {
    if (printable && !isprint((int)ref[i])) printable = 0;
  }
  ...
  if (printable)
    ...
    out.print("0x");
    for (i = 0; i < len; i++) out.print("%02X", (int)ref[i]);
  }
  if (len != (int)sz) {
    out.print("[");
    for (i = len + 1; ref[i] != 0; i++) out.print("%u]", len - i);
    assert((int)sz > i);
...
static const int IntPartDigit_ts =
    ((MaxMySQLDecimalPrecision - MaxMySQLDecimalScale) + DigitsPerDigit_t - 1) /
    DigitsPerDigit_t;
static const int FracPartDigit_ts =
    (MaxMySQLDecimalScale + DigitsPerDigit_t - 1) / DigitsPerDigit_t;
static const int DigitArraySize = IntPartDigit_ts + FracPartDigit_ts;

NdbOut &NdbDictionary::printFormattedValue(NdbOut &out,
  ...
  Uint32 j;
  {
    ...
    if (format.fields_enclosed_by[0] == '\0')
      fields_optionally_enclosed_by = format.fields_optionally_enclosed_by;
    else
    ...
    switch (c->getType()) {
      ...
      out << format.hex_prefix << "0x";
      {
        ...
        /* Write remaining words (less significant) */
          while (k > 0) out.print("%.8X", buf[--k]);
      }
      break;
    }
      case NdbDictionary::Column::Unsigned: {
        if (length > 1) out << format.start_array_enclosure;
        out << *(const Uint32 *)val;
        for (j = 1; j < length; j++) out << " " << *(((const Uint32 *)val) + j);
        if (length > 1) out << format.end_array_enclosure;
      break;
    }
    ...
    case NdbDictionary::Column::Binary:
        if (!format.hex_format) out << fields_optionally_enclosed_by;
      j = c->getLength();
    ...
    case NdbDictionary::Column::Decimal:
      ...
      assert(decimal_string_size(&tmpDec) <= MaxDecimalStrLen);
        ...
        if ((rc = decimal2string(&tmpDec, decStr, &len))) {
          out.print("***Error : bad decimal2string conversion %d ***", rc);
        break;
      }
    ...
    case NdbDictionary::Column::Blob:
      ...
      unsigned len = uint2korr(val_p);
        pretty_print_string(out, format, "Longvarchar", false, val_p + 2, len);
      j = length;
      out << fields_optionally_enclosed_by;
      } break;
      case NdbDictionary::Column::Longvarbinary: {
        if (!format.hex_format) out << fields_optionally_enclosed_by;
      unsigned len = uint2korr(val_p);
        pretty_print_string(out, format, "Longvarbinary", true, val_p + 2, len);
      j = length;
        if (!format.hex_format) out << fields_optionally_enclosed_by;
      } break;
    // fractional time types, see wl#946
      ...
      out.print("%s%02d:%02d:%02d", sign, s.hour, s.minute, s.second);
        if (prec != 0) out.print(".%0*d", prec, s.fraction);
      } break;
    ...
    default: /* no print functions for the rest, just print type */
        out << "Unable to format type (" << (int)c->getType() << ")";
        if (length > 1) out << " " << length << " times";
      break;
    }
    out << format.fields_enclosed_by;
  }
...
NdbOut &NdbDictionary::printColumnTypeDescription(
    NdbOut &out, const NdbDictionary::Column &col) {
  ...
  case NdbDictionary::Column::Olddecimal:
    ...
    break;
  case NdbDictionary::Column::Olddecimalunsigned:
      out << "Olddecimalunsigned(" << col.getPrecision() << ","
          << col.getScale() << ")";
    break;
  case NdbDictionary::Column::Decimal:
...
NdbOut &operator<<(NdbOut &out, const NdbDictionary::Column &col) {
  out << col.getName() << " ";
  ...
  if (col.getDistributionKey()) out << " DISTRIBUTION KEY";
  ...
  if (col.getAutoIncrement()) out << " AUTO_INCR";
  ...
  case NdbDictionary::Column::Text:
    out << " BV=" << col.getBlobVersion();
      out << " BT="
          << ((col.getBlobTable() != nullptr) ? col.getBlobTable()->getName()
                                              : "<none>");
    break;
  default:
    break;
  ...
  if (default_data != nullptr) {
    ...
    NdbDictionary::printFormattedValue(out, f, &col, default_data);
  }
  
  return out;
}

int NdbDictionary::Dictionary::createLogfileGroup(const LogfileGroup &lg,
                                                  ObjectId *obj) {
  int ret;
  DO_TRANS(ret, m_impl.createLogfileGroup(
                    NdbLogfileGroupImpl::getImpl(lg),
                    obj ? &NdbDictObjectImpl::getImpl(*obj) : nullptr));
  return ret;
}

int NdbDictionary::Dictionary::dropLogfileGroup(const LogfileGroup &lg) {
  int ret;
  DO_TRANS(ret, m_impl.dropLogfileGroup(NdbLogfileGroupImpl::getImpl(lg)));
  return ret;
}

NdbDictionary::LogfileGroup NdbDictionary::Dictionary::getLogfileGroup(
    const char *name) {
...
int NdbDictionary::Dictionary::createTablespace(const Tablespace &lg,
                                                ObjectId *obj) {
  int ret;
  DO_TRANS(ret, m_impl.createTablespace(
                    NdbTablespaceImpl::getImpl(lg),
                    obj ? &NdbDictObjectImpl::getImpl(*obj) : nullptr));
  return ret;
}

int NdbDictionary::Dictionary::dropTablespace(const Tablespace &lg) {
  int ret;
  DO_TRANS(ret, m_impl.dropTablespace(NdbTablespaceImpl::getImpl(lg)));
  return ret;
}

NdbDictionary::Tablespace NdbDictionary::Dictionary::getTablespace(
    const char *name) {
...
int NdbDictionary::Dictionary::createDatafile(const Datafile &df, bool force,
                                              ObjectId *obj) {
  int ret;
  DO_TRANS(ret, m_impl.createDatafile(
                    NdbDatafileImpl::getImpl(df), force,
                    obj ? &NdbDictObjectImpl::getImpl(*obj) : nullptr));
  return ret;
}

int NdbDictionary::Dictionary::dropDatafile(const Datafile &df) {
  int ret;
  DO_TRANS(ret, m_impl.dropDatafile(NdbDatafileImpl::getImpl(df)));
  return ret;
}

NdbDictionary::Datafile NdbDictionary::Dictionary::getDatafile(
    Uint32 node, const char *path) {
...
int NdbDictionary::Dictionary::createUndofile(const Undofile &df, bool force,
                                              ObjectId *obj) {
  int ret;
  DO_TRANS(ret, m_impl.createUndofile(
                    NdbUndofileImpl::getImpl(df), force,
                    obj ? &NdbDictObjectImpl::getImpl(*obj) : nullptr));
  return ret;
}

int NdbDictionary::Dictionary::dropUndofile(const Undofile &df) {
  int ret;
  DO_TRANS(ret, m_impl.dropUndofile(NdbUndofileImpl::getImpl(df)));
  return ret;
}

NdbDictionary::Undofile NdbDictionary::Dictionary::getUndofile(
    Uint32 node, const char *path) {
...
void NdbDictionary::Dictionary::invalidateDbGlobal(const char *name) {
  if (m_impl.m_globalHash && name != nullptr) {
...
int NdbDictionary::Dictionary::endSchemaTrans(Uint32 flags) {
...
int NdbDictionary::Dictionary::createHashMap(const HashMap &map,
                                             ObjectId *dst) {
  ObjectId tmp;
  if (dst == nullptr) dst = &tmp;

  int ret;
  DO_TRANS(ret,
           m_impl.m_receiver.create_hashmap(
               NdbHashMapImpl::getImpl(map), &NdbDictObjectImpl::getImpl(*dst),
               0, NdbDictionary::Object::PartitionBalance_ForRPByLDM));
  return ret;
}

int NdbDictionary::Dictionary::createForeignKey(const ForeignKey &fk,
                                                ObjectId *dst, int flags) {
  ObjectId tmp;
  if (dst == nullptr) dst = &tmp;

  if (fk.getParentIndex() == nullptr // primary key
  ...
  DO_TRANS(ret, m_impl.m_receiver.create_fk(NdbForeignKeyImpl::getImpl(fk),
                                       &NdbDictObjectImpl::getImpl(*dst),
                                       implFlags));
  return ret;
}

int NdbDictionary::Dictionary::getForeignKey(ForeignKey &fk, const char *name) {
  return m_impl.m_receiver.get_fk(NdbForeignKeyImpl::getImpl(fk), name);
}

int NdbDictionary::Dictionary::dropForeignKey(const ForeignKey &fk) {
  int ret;
  DO_TRANS(ret, m_impl.m_receiver.drop_fk(NdbForeignKeyImpl::getImpl(fk)));
  return ret;
}
...
void NdbDictionary::Dictionary::print(NdbOut &ndbout,
                                      NdbDictionary::Index const &idx) {
  ndbout << idx;
...
NdbOut &operator<<(class NdbOut &, NdbDictionary::Table const &tab) {
  ...
  ndbout << "Temporary table: " << (tab.getStoredTable() ? "no" : "yes")
         << endl;
...
NdbOut &print_fk_tab_ref(NdbOut &ndbout, const char *fqn) {
  int cnt_slash = 0;
  {
    const char *ptr = fqn;
  ...
  {
    for (; *fqn != '/'; fqn++) ndbout.print("%c", (*fqn));
    ndbout << ".";
...
NdbOut &print_fk_idx_ref(NdbOut &ndbout, const char *fqn) {
  if (fqn == nullptr) {
    ndbout << "PRIMARY KEY";
  } else {
...
void NdbDictionary::Dictionary::print(NdbOut &ndbout,
  ...
  for (j = 0; (int)j < tab.getNoOfPrimaryKeys(); j++) {
    ...
    ndbout << col->getName();
    if ((int)j < tab.getNoOfPrimaryKeys() - 1) ndbout << ", ";
  }
  ...
  for (j = 0; j < list.count; j++) {
    List::Element &elt = list.elements[j];
    if (elt.type != NdbDictionary::Object::ForeignKey) continue;
    ...
    if (getForeignKey(fk, elt.name) == 0) {
      if (strcmp(fk.getChildTable(),
        ...
        for (unsigned i = 0; i < fk.getChildColumnCount(); i++) {
          ...
          if (i + 1 != fk.getChildColumnCount()) ndbout << ", ";
        }
        ndbout << ") REFERENCES ";

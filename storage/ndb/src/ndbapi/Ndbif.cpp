 ...
 ****************************************************************************/
int Ndb::init(int aMaxNoOfTransactions) {
  DBUG_ENTER("Ndb::init");
   ...
   */
  {
    ...
    theImpl->m_node_iter.set_current_pos(theImpl->theCurrentConnectIndex);
  }
  ...
  for (i = 0; i < tMaxNoOfTransactions; i++) {
    thePreparedTransactionsArray[i] = nullptr;
    theSentTransactionsArray[i] = nullptr;
    theCompletedTransactionsArray[i] = nullptr;
  }  // for
  for (i = 0; i < 16; i++) {
    tSignal[i] = getSignal();
    if (tSignal[i] == nullptr) {
      theError.code = 4000;
      goto error_handler;
    }
  }
  for (i = 0; i < 16; i++) releaseSignal(tSignal[i]);

  /* Force visibility of Ndb object initialisation work before marking it
   * initialised */
  theFacade->lock_poll_mutex();
  theFacade->unlock_poll_mutex();
...
void Ndb::connected(Uint32 ref) {
  ...
  DBUG_PRINT(
      ...
      ("connected with ref=%x, id=%d, no_db_nodes=%d, first_trans_id: 0x%lx",
       theMyRef, tmpTheNode, theImpl->theNoOfDBnodes, (long)theFirstTransId));
...
void Ndb::report_node_failure(Uint32 node_id) {
  /**
...
****************************************************************************/
void Ndb::abortTransactionsAfterNodeFailure(Uint16 aNodeId) {
...
void NdbImpl::drop_batched_fragments(
    AssembleBatchedFragments *batched_fragments) {
 ...
 */
Int32 NdbImpl::assemble_data_event_signal(
    AssembleBatchedFragments *batched_fragments, NdbApiSignal *signal,
...
Remark:   One transaction has been completed.
          ...
          up a poller.
******************************************************************************/
void Ndb::completedTransaction(NdbTransaction *aCon) {
  ...
  if ((tNoSentTransactions > 0) &&
      (aCon->theListState == NdbTransaction::InSendList) &&
      (tTransArrayIndex < tNoSentTransactions)) {
    NdbTransaction *tMoveCon =
        theSentTransactionsArray[tNoSentTransactions - 1];
    ...
    theSentTransactionsArray[tNoSentTransactions - 1] = nullptr;
    ...
    if (theImpl->wakeHandler == nullptr) {
...
******************************************************************************/
Uint32 Ndb::pollCompleted(NdbTransaction **aCopyArray) {
  check_send_timeout();
  Uint32 i;
  Uint32 tNoCompletedTransactions = theNoOfCompletedTransactions;
...
void Ndb::check_send_timeout() {
  ...
  if (DBUG_EVALUATE_IF("early_trans_timeout", true, false)) {
    fprintf(stderr,
            "Forcing immediate timeout check in Ndb::check_send_timeout()\n");
  ...
  if (current_time - the_last_check_time > 1000) {
    ...
    for (Uint32 i = 0; i < no_of_sent; i++) {
      ...
      if (DBUG_EVALUATE_IF("early_trans_timeout", true, false)) {
        fprintf(stderr,
                "Inducing early timeout in Ndb::check_send_timeout()\n");
...
void Ndb::remove_sent_list(Uint32 list_index) {
  ...
  theSentTransactionsArray[last_index] = nullptr;
}

Uint32 Ndb::insert_completed_list(NdbTransaction *a_con) {
...
Uint32 Ndb::insert_sent_list(NdbTransaction *a_con) {
  Uint32 no_of_sent = theNoOfSentTransactions;
...
******************************************************************************/
void Ndb::sendPrepTrans(int forceSend) {
  ...
  */
  Uint32 i;
  theCachedMinDbNodeVersion =
      theImpl->m_transporter_facade->getMinDbNodeVersion();
  ...
  for (i = 0; i < no_of_prep_trans; i++) {
    ...
    if ((theImpl->getNodeSequence(node_id) == a_con->theNodeSequence) &&
        (theImpl->get_node_alive(node_id) ||
         theImpl->get_node_stopping(node_id))) {
      /*
      We will send if
...
/*****************************************************************************
void waitCompletedTransactions(int aMilliSecondsToWait, int
noOfEventsToWaitFor);
...
Remark:   First send all prepared operations and then check if there are any
          ...
          transactions.
******************************************************************************/
void Ndb::waitCompletedTransactions(int aMilliSecondsToWait,
  ...
  do {
    ...
    waitTime = aMilliSecondsToWait - 
      (int)NdbTick_Elapsed(theImpl->m_start_time,wait_end).milliSec();
#ifndef NDEBUG
    if (DBUG_EVALUATE_IF("early_trans_timeout", true, false)) {
      fprintf(stderr,
              "Inducing early timeout in Ndb::waitCompletedTransactions()\n");
      break;
    }
#endif
  } while (waitTime > 0);
  theImpl->recordWaitTimeNanos(NdbTick_Elapsed(theImpl->m_start_time,
                                               wait_end).nanoSec());
...
Remark:   First send all prepared operations and then check if there are any
          ...
          transactions.
******************************************************************************/
void Ndb::sendPreparedTransactions(int forceSend) {
  theImpl->lock();
...
/*****************************************************************************
int sendPollNdb(int aMillisecondNumber, int minNoOfEventsToWakeup = 1, int
forceSend = 0);
...
******************************************************************************/
int Ndb::sendPollNdb(int aMillisecondNumber, int minNoOfEventsToWakeup,
                     int forceSend) {
  /*
...
int Ndb::poll_trans(int aMillisecondNumber, int minNoOfEventsToWakeup,
                    PollGuard *pg) {
...
******************************************************************************/
int Ndb::pollNdb(int aMillisecondNumber, int minNoOfEventsToWakeup) {
  /*
    The PollGuard has an implicit call of unlock_and_signal through the
    ~PollGuard method. This method is called implicitly by the compiler
    in all places where the object is out of context due to a return,
...
void NdbTransaction::sendTC_COMMIT_ACK(NdbImpl *impl, NdbApiSignal *aSignal,
                                       ...
                                       Uint32 aTCRef, bool send_immediate) {
#ifdef MARKER_TRACE
...
int NdbImpl::send_dump_state_all(Uint32 *dumpStateCodeArray, Uint32 len) {
...
void NdbImpl::init_dump_state_signal(NdbApiSignal *aSignal,
                                     Uint32 *dumpStateCodeArray, Uint32 len) {
...
int NdbImpl::send_event_report(bool is_poll_owner, Uint32 *data,
                               Uint32 length) {
  NdbApiSignal aSignal(m_ndb.theMyRef);
  aSignal.theTrace = TestOrd::TraceAPI;
  aSignal.theReceiversBlockNumber = CMVMI;
 ...
 * all nodes.
 */
int NdbImpl::send_to_nodes(NdbApiSignal *aSignal, bool is_poll_owner,
                           bool send_to_all) {
  int ret;
  Uint32 tNode;
  ...
  while ((tNode = m_ndb_cluster_connection.get_next_node_any(node_iter))) {
 ...
 */
int NdbImpl::send_to_node(NdbApiSignal *aSignal, Uint32 tNode,
                          bool is_poll_owner) {
  int ret_code = 1;

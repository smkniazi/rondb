...
inline static void prefetch_scan_record_3(Uint32 *scan_ptr) {
  NDB_PREFETCH_WRITE(scan_ptr);
  NDB_PREFETCH_WRITE(scan_ptr + 16);
  NDB_PREFETCH_WRITE(scan_ptr + 32);
}

void Dbtux::prepare_scan_ctx(Uint32 scanPtrI) {
  jamDebug();
  FragPtr fragPtr;
  ScanOpPtr scanPtr;
  IndexPtr indexPtr;
 ...
 */
void Dbtux::prepare_move_scan_ctx(ScanOpPtr scanPtr, Dbtux *tux_block) {
 ...
 */
void Dbtux::prepare_build_ctx(TuxCtx &ctx, FragPtr fragPtr) {
  IndexPtr indexPtr;
  ctx.fragPtr = fragPtr;
  indexPtr.i = fragPtr.p->m_indexId;
  c_indexPool.getPtr(indexPtr);
 ...
 */
void Dbtux::prepare_scan_bounds(const ScanOp *scanPtrP, const Index *indexPtrP,
                                Dbtux *tux_block) {
  jamDebug();
  ...
  if (likely(scanBound.m_cnt != 0)) {
    ...
    KeyDataArray *key_data = new (&c_ctx.searchScanDataArray) KeyDataArray();
    key_data->init_bound(searchBound, scanBound.m_cnt);
    KeyBoundArray *searchBoundArray = new (&c_ctx.searchScanBoundArray)
        KeyBoundArray(&index.m_keySpec, key_data, scanBound.m_side);
...
void Dbtux::execACC_CHECK_SCAN(Signal *signal) {
  jamEntryDebug();
  ...
  if (req->checkLcpStop == AccCheckScan::ZCHECK_LCP_STOP &&
      (scan.m_lockwait || wait_scan_lock_record)) {
    /**
 ...
 */
void Dbtux::execACC_SCANREQ(Signal *signal) {
  jamEntry();
  ...
  do {
    ...
    if (ERROR_INSERTED(12008) || !c_scanOpPool.seize(scanPtr)) {
 ...
 */
void Dbtux::execTUX_BOUND_INFO(Signal *signal) {
  jamEntry();
  // get records
...
void Dbtux::execNEXT_SCANREQ(Signal *signal) {
...
#ifdef VM_TRACE
  if (debugFlags & DebugScan) {
    tuxDebugOut << "NEXT_SCANREQ scan " << c_ctx.scanPtr.i << " " << scan
                << endl;
  }
#endif
...
void Dbtux::continue_scan(Signal *signal, ScanOpPtr scanPtr, Frag &frag,
  ...
  // check index online
  if (unlikely(index.m_state != Index::Online) && scan.m_errorCode == 0) {
    jam();
#ifdef VM_TRACE
    if (debugFlags & (DebugMeta | DebugScan)) {
  ...
  if (scan.m_state == ScanOp::Current || scan.m_state == ScanOp::Next) {
    jamDebug();
    // look for next
  ...
  if (likely(scan.m_state == ScanOp::Found)) {
    ...
    if (unlikely(!scan.m_readCommitted)) {
      ...
      AccLockReq *const lockReq = (AccLockReq *)signal->getDataPtrSend();
      lockReq->returnCode = RNIL;
      lockReq->requestInfo = scan.m_lockMode == 0 ? AccLockReq::LockShared
      ...
      switch (lockReq->returnCode) {
        case AccLockReq::Success: {
          ...
          if (debugFlags & (DebugScan | DebugLock)) {
            tuxDebugOut << "Lock immediate scan " << scanPtr.i << " " << scan
                        << endl;
          }
#endif
          if (ttl_table) {
...
#ifdef VM_TRACE
          if (debugFlags & (DebugScan | DebugLock)) {
            tuxDebugOut << "Lock wait scan " << scanPtr.i << " " << scan
                        << endl;
          }
#endif
          // LQH will wake us up
        ...
        case AccLockReq::Refused: {
          ...
          g_eventLogger->info("(%u) Refused tab(%u,%u) row(%u,%u)", instance(),
                              scan.m_tableId, frag.m_fragId, lkey1, lkey2);
          ndbassert(false);
          // skip it
 ...
 */
void Dbtux::execACCKEYCONF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
 ...
 */
void Dbtux::execACCKEYREF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
  scanPtr.i = signal->theData[0];
  ndbrequire(c_scanOpPool.getValidPtr(scanPtr));
 ...
 */
void Dbtux::execACC_ABORTCONF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
  scanPtr.i = signal->theData[0];
  ndbrequire(c_scanOpPool.getValidPtr(scanPtr));
...
void Dbtux::scanFirst(ScanOpPtr scanPtr, Frag &frag, const Index &index) {
  ...
  KeyBoundArray *searchBoundArray =
      new (&c_ctx.searchKeyBoundArray) KeyBoundArray(
          &index.m_keySpec, &c_ctx.searchKeyDataArray, scanBound.m_side);
 ...
 */
void Dbtux::scanFind(ScanOpPtr scanPtr, Frag &frag) {
  ScanOp &scan = *scanPtr.p;
 ...
 */
Uint32 Dbtux::scanNext(ScanOpPtr scanPtr, bool fromMaintReq, Frag &frag) {
  ScanOp &scan = *scanPtr.p;
...
bool Dbtux::checkScanInstance(Uint32 scanInstance) {
 ...
 * invalid data.
 */
inline bool Dbtux::scanCheck(ScanOp &scan, TreeEnt ent) {
  jamDebug();
  ...
  if (likely(scanBoundCnt != 0)) {
    ...
    ret = c_tup->tuxReadAttrsCurr(c_ctx.jamBuffer, keyAttrs32, count,
                                  outputBuffer, tupVersion);
 ...
 */
bool Dbtux::scanVisible(ScanOp &scan, TreeEnt ent) {
  ...
  bool ret = c_tup->tuxQueryTh(opPtrI, tupVersion, transId1, transId2, dirty,
                               savePointId);
  jamEntryDebug();
  return ret;
}
 ...
 * send a REF.
 */
void Dbtux::scanClose(Signal *signal, ScanOpPtr scanPtr) {
  ScanOp &scan = *scanPtr.p;
  ndbrequire(!scan.m_lockwait && scan.m_accLockOp == RNIL);
  ...
  if (scanPtr.p->m_errorCode == 0) {
    ...
    releaseScanOp(scanPtr);
    EXECUTE_DIRECT(blockNo, GSN_NEXT_SCANREF, signal,
 ...
 */
void Dbtux::releaseScanOp(ScanOpPtr &scanPtr) {
#ifdef VM_TRACE
  if (debugFlags & DebugScan) {
  ...
  checkPoolShrinkNeed(DBTUX_SCAN_BOUND_TRANSIENT_POOL_INDEX, c_scanBoundPool);

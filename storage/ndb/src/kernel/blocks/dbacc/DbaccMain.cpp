...
void Dbacc::execACCKEY_ORD_no_ptr(Signal *signal, Uint32 opPtrI) {
  OperationrecPtr opPtr;
  opPtr.i = opPtrI;
  ndbrequire(oprec_pool.getValidPtr(opPtr));
...
ref:
              ...
              (nextbits & Operationrec::OP_LOCK_REQ) == 0))
  {
    ...
    sendSignal(nextOp.p->userblockref, GSN_ACCKEYREF, signal, 2, JBB);
  }
...
/* ------------------------------------------------------------------------ */
Uint32 Dbacc::getNoParallelTransaction(const Operationrec *op) const {
  OperationrecPtr tmp;
...
#ifdef VM_TRACE
Uint32 Dbacc::getNoParallelTransactionFull(Operationrec *op) const {
  OperationrecPtr tmp;
...
bool Dbacc::validate_parallel_queue(OperationrecPtr opPtr,
...
bool Dbacc::validate_lock_queue(OperationrecPtr opPtr) const {
  if (m_is_query_block) {
    return true;
  }
  ...
  if (loPtr.p->nextSerialQue != RNIL) {
    ...
    while (true)
    {
      ...
      vlqrequire((lastS.p->m_op_bits & Operationrec::OP_STATE_MASK) ==
                 Operationrec::OP_STATE_WAITING);
      if (lastS.p->nextSerialQue == RNIL) break;
      prev = lastS.i;
...
NdbOut &operator<<(NdbOut &out, Dbacc::OperationrecPtr ptr) {
  Uint32 opbits = ptr.p->m_op_bits;
  out << "[ " << dec << ptr.i << " [ " << hex << ptr.p->transId1 << " " << hex
      << ptr.p->transId2 << "] "
      << " bits: H'" << hex << opbits << " ";
  ...
  switch (opbits & Dbacc::Operationrec::OP_MASK) {
    ...
    case ZSCAN_OP:
      ...
      break;
    default:
      out << "<Unknown: H'" << hex << (opbits & Dbacc::Operationrec::OP_MASK)
          << "> ";
  }
  ...
  switch (opbits & Dbacc::Operationrec::OP_STATE_MASK) {
    ...
    default:
      out << " <Unknown: H'" << hex
          << (opbits & Dbacc::Operationrec::OP_STATE_MASK) << "> ";
  }
  ...
  /*
      ...
      ,OP_INITIAL             = ~(Uint32)0
  */
  if (opbits & Dbacc::Operationrec::OP_LOCK_OWNER) out << "LO ";

  if (opbits & Dbacc::Operationrec::OP_DIRTY_READ) out << "DR ";

  if (opbits & Dbacc::Operationrec::OP_LOCK_REQ) out << "LOCK_REQ ";

  if (opbits & Dbacc::Operationrec::OP_COMMIT_DELETE_CHECK)
    out << "COMMIT_DELETE_CHECK ";

  if (opbits & Dbacc::Operationrec::OP_INSERT_IS_DONE) out << "INSERT_IS_DONE ";

  if (opbits & Dbacc::Operationrec::OP_ELEMENT_DISAPPEARED)
    out << "ELEMENT_DISAPPEARED ";

  if (opbits & Dbacc::Operationrec::OP_PENDING_ABORT) out << "PENDING_ABORT ";
...
void Dbacc::dump_lock_queue(OperationrecPtr loPtr) const {
  if (m_is_query_block) {
    return;
  }
 ...
 * ------------------------------------------------------------------------- */
Uint32 Dbacc::placeWriteInLockQueue(OperationrecPtr lockOwnerPtr) const {
  OperationrecPtr lastOpPtr;
  lastOpPtr.i = lockOwnerPtr.p->m_lo_last_parallel_op_ptr_i;
  Uint32 opbits = operationRecPtr.p->m_op_bits;
...
Uint32 Dbacc::placeReadInLockQueue(OperationrecPtr lockOwnerPtr) const {
  OperationrecPtr lastOpPtr;
  OperationrecPtr loopPtr = lockOwnerPtr;
  lastOpPtr.i = lockOwnerPtr.p->m_lo_last_parallel_op_ptr_i;
  Uint32 opbits = operationRecPtr.p->m_op_bits;
...
/* ******************----------------------------------------------------- */
void Dbacc::execACCMINUPDATE(Signal *signal, Uint32 opPtrI,
                             Dbacc::Operationrec *opPtrP, Uint32 page_no,
                             Uint32 page_idx) {
...
void Dbacc::removerow(Uint32 opPtrI, const Local_key *key) {
  jamEntry();
  operationRecPtr.i = opPtrI;
...
void Dbacc::execACC_COMMITREQ(Signal *signal, Uint32 opPtrI,
  ...
  operationRecPtr.p->m_op_bits = Operationrec::OP_INITIAL;
  if ((Toperation != ZREAD) && (Toperation != ZSCAN_OP)) {
...
/* ******************------------------------------+                         */
/*   SENDER: LQH,    LEVEL B       */
void Dbacc::execACC_ABORTREQ(Signal *signal, Uint32 opPtrI,
                             Dbacc::Operationrec *opPtrP, Uint32 sendConf) {
  jamEntry();
  operationRecPtr.i = opPtrI;
  operationRecPtr.p = opPtrP;
  fragrecptr.i = operationRecPtr.p->fragptr;
...
void Dbacc::execACC_LOCKREQ(Signal *signal) {
  ...
  if (lockOp == AccLockReq::LockShared || lockOp == AccLockReq::LockExclusive) {
    jam();
    fragrecptr = prepare_fragptr;
    ...
    if (likely(succ)) {
      ...
      execACCKEYREQ(signal, operationRecPtr.i, operationRecPtr.p);
  ...
  if (lockOp == AccLockReq::Unlock)
  {
    ...
    execACC_COMMITREQ(signal, operationRecPtr.i, operationRecPtr.p);
    releaseOpRec();
    req->returnCode = AccLockReq::Success;
    *sig = *req;
    return;
...
void Dbacc::insertElement(const Element   elem,
  ...
  do {
    insertContainer(elem, oprecptr, pageptr, conidx, isforward, conptr,
  ...
  addnewcontainer(pageptr, conptr, newPageindex, newBuftype, nextOnSamePage,
                  inrNewPageptr.i);
  ...
  insertContainer(elem, oprecptr, pageptr, conidx, isforward, conptr,
                  containerhead, conScanMask, true, tidrResult);
 ...
 */
void Dbacc::insertContainer(const Element elem, const OperationrecPtr oprecptr,
                            const Page8Ptr pageptr, const Uint32 conidx,
                            const bool isforward, Uint32 &conptr,
                            ContainerHeader &containerhead, Uint16 conScanMask,
                            const bool newContainer, Uint32 &result) {
 ...
 * ------------------------------------------------------------------------- */
void Dbacc::addnewcontainer(Page8Ptr pageptr, Uint32 conptr, Uint32 nextConidx,
...
void Dbacc::increaselistcont(Page8Ptr ilcPageptr) {
  ...
  if (ilcPageptr.p->word32[Page8::ALLOC_CONTAINERS] == ZFREE_LIMIT + 1) {
    ...
    if (((ilcPageptr.p->word32[Page8::EMPTY_LIST] >> ZPOS_PAGE_TYPE_BIT) & 3) ==
        1) {
      jam();
      LocalContainerPageList sparselist(c_page8_pool,
                                        fragrecptr.p->sparsepages);
...
void Dbacc::seizeRightlist(Page8Ptr slPageptr, Uint32 tslPageindex) {
  ...
  if (tslPrevfree == Container::NO_CONTAINER_INDEX) {
    ...
    slPageptr.p->word32[Page8::EMPTY_LIST] =
        ((tsrlTmp >> 7) << 7) | tslNextfree;
  } else {
    ndbrequire(tslPrevfree <= Container::MAX_CONTAINER_INDEX);
    jam();
...
void Dbacc::getdirindex(Page8Ptr &pageptr, Uint32 &conidx) {
  ...
  pageptr.i =
      getPagePtr(fragrecptr.p->directory, fragrecptr.p->getPageNumber(address));
  c_page8_pool.getPtr(pageptr);
}  // Dbacc::getdirindex()

Uint32 Dbacc::find_key_operation(Ptr<Operationrec> opPtr,
 ...
 * ------------------------------------------------------------------------- */
Uint32 Dbacc::getElement(const AccKeyReq *signal, OperationrecPtr &lockOwnerPtr,
          ...
          if (!searchLocalKey) {
            ...
            Uint32 len =
                readTablePk(localkey.m_page_no, localkey.m_page_idx,
                            tgeElementHeader, lockOwnerPtr, &keys[0], xfrm);
            if (unlikely(len == 0)) {
              jamDebug();
              found = false;
            } else {
 ...
 */
void Dbacc::report_pending_dealloc(Signal *signal, Operationrec *opPtrP,
 ...
 */
void Dbacc::trigger_dealloc(Signal *signal, const Operationrec *opPtrP) {
  Local_key localKey = opPtrP->localdata;
  Uint32 opbits = opPtrP->m_op_bits;
  Uint32 userptr = opPtrP->userptr;
  const bool scanInd = ((opbits & Operationrec::OP_MASK) == ZSCAN_OP) ||
...
void Dbacc::commitdelete(Signal *signal) {
  ...
  while (lastPageptr.i != delPageptr.i || tlastContainerptr != delConptr) {
   ...
   * should have been fewer than delete containers which is not allowed for
   * last.
   */
  if ((lastPageptr.i == delPageptr.i) && (tlastContainerptr == delConptr)) {
 ...
 * ------------------------------------------------------------------------- */
void Dbacc::deleteElement(Page8Ptr delPageptr, Uint32 delConptr,
                          Uint32 delElemptr, Page8Ptr lastPageptr,
                          Uint32 lastElemptr) const {
  OperationrecPtr deOperationRecPtr;

  if (lastElemptr >= 2048) goto deleteElement_index_error1;
  {
                             ...
                             Uint32& tlastElementptr)
{
  ...
  while (containerhead.getNextEnd() != 0) {
    ...
    ndbassert(tlastContainerlen >=
              ((Uint32)Container::HEADER_SIZE + fragrecptr.p->elementLength));
  }
  /**
  ...
  if (lastIsforward) {
    ...
    jam();
    tlastElementptr = (tlastContainerptr +
                       (Container::HEADER_SIZE - fragrecptr.p->elementLength)) -
                      tlastContainerlen;
  }  // if
  if (containerhead.isUsingBothEnds()) {
  ...
  if (tlastContainerlen <= Container::HEADER_SIZE) {
    ...
    if (lastPrevpageptr.i != RNIL) {
      ...
      if (containerhead.isScanInProgress())
      {
        ...
        scansInProgress =
            lastPageptr.p->checkScans(scansInProgress, tlastContainerptr);
        Uint16 scanbit = 1;
        for (int i = 0; scansInProgress != 0;
 ...
 */
void Dbacc::mark_pending_abort(OperationrecPtr abortingOp,
                               Uint32 nextParallelOp) {
  jam();
  ...
  if (opType == ZREAD || opType == ZSCAN_OP) {
    jam();
    return;
  }
  ...
  ndbassert(opType == ZINSERT || opType == ZUPDATE ||
 ...
 */
bool Dbacc::checkOpPendingAbort(Uint32 accConnectPtr) const {
  OperationrecPtr opPtr;
                                ...
                                Uint32 hash)
{
...

  {
    ...
    frp.p->m_lockStats.wait_fail(
        (opbits & Operationrec::OP_LOCK_MODE) != ZREADLOCK, opPtr.p->m_lockTime,
        getHighResTimer());
  }
...
void Dbacc::commitDeleteCheck(Signal *signal) {
  OperationrecPtr opPtr;
  OperationrecPtr lastOpPtr;
                         ...
                         Uint32 hash)
{
  ...
  enum { NOTHING, CHECK_LOCK_UPGRADE, START_NEW } action = NOTHING;
...
ref:
  ...
  signal->theData[1] = errCode;
  sendSignal(newOwner.p->userblockref, GSN_ACCKEYREF, signal, 2, JBB);
...
void Dbacc::execEXPANDCHECK2(Signal *signal) {
  ...
  c_tup->prepare_tab_pointers_acc(fragrecptr.p->myTableId, fragrecptr.p->myfid);

...
static void init_ndbrecord_info(Ndb &myNdb) {
  ...
  psecondaryIndexRecord =
      myDict->createRecord(mySIndex, recordSpec, 2, sizeof(recordSpec[0]));
...
static void do_mixed_update(Ndb &myNdb) {
  ...
  for (int i = 0; i < 10; i += 2) {
    ...
    const NdbOperation *pop = myTransaction->updateTuple(
        pkeyColumnRecord, (char *)&row, pallColsRecord, (char *)&row, &attrMask,
        &opts);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    const NdbOperation *pop =
        myTransaction->readTuple(pkeyColumnRecord, (char *)&rowData,
                                 ...
                                 (char *)&rowData, NdbOperation::LM_Read,
...
static void do_scan(Ndb &myNdb, ApiType accessType) {
  ...
  switch (accessType) {
    ...
    case api_record: {
       ...
       * row from the scan
       */
      while ((rc = psop->nextResult((const char **)&prowData, true, false)) ==
             0) {
...
static void do_mixed_scan(Ndb &myNdb) {
  ...
  psop =
      myTransaction->scanTable(pallColsRecord, NdbOperation::LM_Read, &attrMask,
                               &options, sizeof(NdbScanOperation::ScanOptions));
  if (psop == NULL) APIERROR(myTransaction->getNdbError());
  ...
  while ((rc = psop->nextResult((const char **)&prowData, true, false)) == 0) {
    printf(" %2d    %2d    %2d\n", prowData->attr1, prowData->attr2,
  ...
  switch (accessType) {
    ...
    case api_record: {
      ...
      psop = myTransaction->scanIndex(
          pkeyIndexRecord, pallColsRecord, NdbOperation::LM_Read,
  ...
  switch (accessType) {
    case api_attr: {
      while (psop->nextResult(true) == 0) {
        printf(" %2d    %2d    %2d    Range no : %2d\n",
               ...
               recAttrAttr3->u_32_value(), psop->get_range_no());
      }
    ...
    case api_record: {
      ...
      while ((rc = psop->nextResult((const char **)&prowData, true, false)) ==
        ...
        // printf(" PTR : %d\n", (int) prowData);
        printf(" %2d    %2d    %2d    Range no : %2d\n", prowData->attr1,
               prowData->attr2, prowData->attr3, psop->get_range_no());
      }
...
static void do_mixed_indexScan(Ndb &myNdb) {
  ...
  while ((rc = psop->nextResult((const char **)&prowData, true, false)) == 0) {
    printf(" %2d    %2d    %2d    Range no : %2d\n", prowData->attr1,
           prowData->attr2, recAttrAttr3->u_32_value(), psop->get_range_no());
  }
...
static void do_read_and_delete(Ndb &myNdb) {
  ...
  const NdbOperation *pop = myTransaction->deleteTuple(
      pkeyColumnRecord,  // Spec of key used
...
static void do_scan_update(Ndb &myNdb, ApiType accessType) {
  ...
  switch (accessType) {
    ...
    case api_record: {
      ...
      while (result == NEED_TO_FETCH_ROWS) {
        bool fetch = true;
        while ((result = psop->nextResult((const char **)&prowData, fetch,
                                          false)) == GOT_ROW) {
          fetch = false;
          ...
          /* Update it */
          const NdbOperation *op = psop->updateCurrentTuple(
              myTransaction, pallColsRecord, (char *)&r);
...
static void do_scan_delete(Ndb &myNdb, ApiType accessType) {
  ...
  switch (accessType) {
    ...
    case api_record: {
      ...
      psop = myTransaction->scanTable(pkeyColumnRecord, NdbOperation::LM_Read,
                                      NULL,  // mask
                                      &options,
                                      sizeof(NdbScanOperation::ScanOptions));
      ...
      while (result == NEED_TO_FETCH_ROWS) {
        ...
        while ((result = psop->nextResult((const char **)&prowData, fetch,
          ...
          if (r.attr1 == 2) {
            ...
            theDeleteOp = psop->deleteCurrentTuple(
                myTransaction, pallColsRecord, (char *)&readRow, &attrMask,
                &options, sizeof(NdbOperation::OperationOptions));
      ...
      while (result == NEED_TO_FETCH_ROWS) {
        ...
        while ((result = psop->nextResult((const char **)&prowData, fetch,
          ...
          if (r.attr1 == 9) {
            ...
            const NdbOperation *lockOp = psop->lockCurrentTuple(
                myTransaction, pallColsRecord, (char *)&rereadData, &attrMask,
                &options, sizeof(NdbOperation::OperationOptions));
        ...
        if (processed != 0) {
          ...
          printf("ATTR1  ATTR2  ATTR3 COMMITS\n");
          printf("  %2d    %2d    %2d    %2d\n", rereadData.attr1,
                 rereadData.attr2, attr3->u_32_value(),
                 commitCount->u_32_value());
        }
      }
...
static void do_all_extras_read(Ndb &myNdb) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    const NdbOperation *pop = myTransaction->readTuple(
        pkeyColumnRecord, (char *)&rowData, pkeyColumnRecord,
        NULL,  // null result row
        NdbOperation::LM_Read, &attrMask, &opts);
    if (pop == NULL) APIERROR(myTransaction->getNdbError());
...
static void do_secondary_indexScan(Ndb &myNdb, ApiType accessType) {
  ...
  switch (accessType) {
    ...
    case api_record: {
      ...
      psop = myTransaction->scanIndex(
          psecondaryIndexRecord, pallColsRecord, NdbOperation::LM_Read,
          NULL,  // mask
          NULL,  // bound
          &options, sizeof(NdbScanOperation::ScanOptions));
...
static void do_interpreted_update(Ndb &myNdb, ApiType accessType) {
  ...
  switch (accessType) {
    case api_attr: {
       ...
       */
      // R5= 3
      if (pop->load_const_u32(R5, 3) != 0) APIERROR(pop->getNdbError());

      // R1= *ATTR1; R2= *ATTR2; R3= *ATTR3
      if (pop->read_attr("ATTR1", R1) != 0) APIERROR(pop->getNdbError());
      if (pop->read_attr("ATTR2", R2) != 0) APIERROR(pop->getNdbError());
      if (pop->read_attr("ATTR3", R3) != 0) APIERROR(pop->getNdbError());
      ...
      // *ATTR2= R2
      if (pop->write_attr("ATTR2", R2) != 0) APIERROR(pop->getNdbError());

      // *ATTR3= R3
      if (pop->write_attr("ATTR3", R3) != 0) APIERROR(pop->getNdbError());

      // *ATTR3 = *ATTR3 - 30
      if (pop->subValue("ATTR3", (Uint32)30) != 0) APIERROR(pop->getNdbError());

      Uint32 comparisonValue = 10;

      // if *ATTR3 > comparisonValue, goto Label 0
      if (pop->branch_col_lt(pattr3Col->getColumnNo(), &comparisonValue,
                             sizeof(Uint32), false, 0) != 0)
        APIERROR(pop->getNdbError());

      // assert(false)
      // Fail the operation with error 627 if we get here.
      if (pop->interpret_exit_nok(627) != 0) APIERROR(pop->getNdbError());
      ...
      if (pop->branch_col_eq(pattr2Col->getColumnNo(), &comparisonValue2,
                             sizeof(Uint32), false, 1) != 0)
        APIERROR(pop->getNdbError());

      // assert(false)
      // Fail the operation with error 628 if we get here
      if (pop->interpret_exit_nok(628) != 0) APIERROR(pop->getNdbError());
      ...
      // R1 = 10
      if (pop->load_const_u32(R1, 10) != 0) APIERROR(pop->getNdbError());

      // R3 = 2
      if (pop->load_const_u32(R3, 2) != 0) APIERROR(pop->getNdbError());
      ...
      // R4= 1
      if (pop->load_const_u32(R4, 1) != 0) APIERROR(pop->getNdbError());
      ...
      if (pop->interpret_exit_nok(629) != 0) APIERROR(pop->getNdbError());
    ...
    case api_record: {
       ...
       */
      const NdbOperation *op0 = myTransaction->readTuple(
          pkeyColumnRecord, (char *)&rowData, pallColsRecord, (char *)&rowData);
      ...
      // R5= 3
      if (code->load_const_u32(R5, 3) != 0) APIERROR(code->getNdbError());

      // R1= *ATTR1; R2= *ATTR2; R3= *ATTR3
      if (code->read_attr(R1, pattr1Col) != 0) APIERROR(code->getNdbError());
      if (code->read_attr(R2, pattr2Col) != 0) APIERROR(code->getNdbError());
      if (code->read_attr(R3, pattr3Col) != 0) APIERROR(code->getNdbError());

      // R3= R3-R5
      if (code->sub_reg(R3, R3, R5) != 0) APIERROR(code->getNdbError());

      // R2= R1+R2
      if (code->add_reg(R2, R1, R2) != 0) APIERROR(code->getNdbError());

      // *ATTR2= R2
      if (code->write_attr(pattr2Col, R2) != 0) APIERROR(code->getNdbError());

      // *ATTR3= R3
      if (code->write_attr(pattr3Col, R3) != 0) APIERROR(code->getNdbError());
      ...
      if (code->interpret_exit_nok(627) != 0) APIERROR(code->getNdbError());
      ...
      if (code->interpret_exit_nok(628) != 0) APIERROR(code->getNdbError());

      // Label 1
      if (code->def_label(1) != 0) APIERROR(code->getNdbError());

      // R1= 10
      if (code->load_const_u32(R1, 10) != 0) APIERROR(code->getNdbError());

      // R3= 2
      if (code->load_const_u32(R3, 2) != 0) APIERROR(code->getNdbError());
      ...
      // Return ok
      if (code->interpret_exit_ok() != 0) APIERROR(code->getNdbError());

      // Subroutine 0
      if (code->def_sub(0) != 0) APIERROR(code->getNdbError());

      // R4= 1
      if (code->load_const_u32(R4, 1) != 0) APIERROR(code->getNdbError());

      // Label 2
      if (code->def_label(2) != 0) APIERROR(code->getNdbError());

      // R3= R3-R4
      if (code->sub_reg(R3, R3, R4) != 0) APIERROR(code->getNdbError());

      // R2= R2+R1
      if (code->add_reg(R2, R2, R1) != 0) APIERROR(code->getNdbError());

      // Loop, subtracting 1 from R4 until R4>1
      if (code->branch_ge(R3, R4, 2) != 0) APIERROR(code->getNdbError());

      // Jump to label 3
      if (code->branch_label(3) != 0) APIERROR(code->getNdbError());

      // Fail operation with error 629
      if (code->interpret_exit_nok(629) != 0) APIERROR(code->getNdbError());
      ...
      // R6= R1+R2
      if (code->add_reg(R6, R1, R2) != 0) APIERROR(code->getNdbError());
      ...
      pop = myTransaction->updateTuple(
          pkeyColumnRecord, (char *)&rowData, pallColsRecord, (char *)&rowData,
          (const unsigned char *)&mask,  // mask - update nothing
          &oo, sizeof(NdbOperation::OperationOptions));
      if (pop == NULL) APIERROR(myTransaction->getNdbError());
  ...
  switch (accessType) {
    ...
    case api_record: {
      printf(
          " %2d    %2d    %2d  Before\n"
          " %2d    %2d    %2d  After\n",
          rowData.attr1, rowData.attr2, rowData.attr3, rowData2.attr1,
          rowData2.attr2, rowData2.attr3);
      break;
    }
    default: {
      std::cout << "Bad branch : " << accessType << "\n";
...
static void do_interpreted_scan(Ndb &myNdb, ApiType accessType) {
  ...
  if (code->interpret_exit_nok() != 0) APIERROR(myTransaction->getNdbError());

  /* Label 0 */
  if (code->def_label(0) != 0) APIERROR(myTransaction->getNdbError());

  /* Return this row */
  if (code->interpret_exit_ok() != 0) APIERROR(myTransaction->getNdbError());
  ...
  switch (accessType) {
    ...
    case api_record: {
      ...
      psop =
          myTransaction->scanTable(pallColsRecord, NdbOperation::LM_Read,
                                   NULL,  // mask
                                   &so, sizeof(NdbScanOperation::ScanOptions));
...
static void do_read_using_default(Ndb &myNdb) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    *((unsigned int *)attr1) = i;
    ...
    const NdbOperation *pop =
        myTransaction->readTuple(tableRec, buffer,
    ...
    if (err.code != 0) {
      if (err.classification == NdbError::NoDataFound)
        std::cout << "Detected that tuple " << i << " doesn't exist!"
                  << std::endl;
      else
        APIERROR(myTransaction->getNdbError());
    } else {

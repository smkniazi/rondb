...
#ifndef NDB_RECORD
    pkAttrIds = (Uint32*)
      amalloc->alloc_bytes(numPrimaryKeys *
        sizeof(Uint32), 8);
#endif
    key_op->m_readColumns = readCols;
#ifndef NDB_RECORD
    key_op->m_rec_attrs = recAttrs;
    key_op->m_PkAttrIds = pkAttrIds;
    key_op->m_ReadAttrIds = readAttrIds;
#endif
#ifdef NDB_RECORD
...
start:
  for (Uint32 opIdx = 0; opIdx < m_numOperations; opIdx++) {
    ...
    NdbOperation *operation = m_ndbTransaction->getNdbOperation(tableDict);
    if (unlikely(operation == nullptr)) {
      return RS_RONDB_SERVER_ERROR(m_ndbTransaction->getNdbError(), ERROR_007);
    }
    key_op->m_ndbOperation = operation;
    ...
    Uint64 *pk_buf[4096/8 + 1];
    const char *pk_ptr = (const char*)&pk_buf[0];
    for (Uint32 colIdx = 0; colIdx < numPrimaryKeys; colIdx++) {
      RS_Status status =
        set_operation_pk_col(key_op->m_pkColumns[colIdx],
                             req,
      ...
      int retVal =
        operation->equal(key_op->m_PkAttrIds[colIdx], pk_ptr);
      if (unlikely(retVal != 0)) {
        return RS_SERVER_ERROR(ERROR_023);
      }
    }
    Uint32 num_read_columns = key_op->m_num_read_columns;
    for (Uint32 i = 0; i < num_read_columns; i++) {
      const NdbDictionary::Column *col = key_op->m_readColumns[i];
      Uint32 col_id = col->getColumnNo();
      NdbRecAttr *recAttr = operation->getValue(col_id, nullptr);
      if (unlikely(recAttr == nullptr)) {
        return RS_SERVER_ERROR(
          ERROR_037 + std::string(" Column: ") + std::string(col->getName()));
      }
      key_op->m_rec_attrs[i] = recAttr;
    }
#endif
...
#else
      RS_Status ret = key_op->append_op_recs(resp);
#endif
...
RS_Status KeyOperation::write_col_to_resp(Uint32 colIdx,
  ...
  const NdbDictionary::Column *col = m_readColumns[colIdx];
  const NdbRecAttr *recAttr = m_rec_attrs[colIdx];
  const char *col_name = col->getName();
  if (recAttr->isNULL()) {
    return response->SetColumnDataNull(col_name);
  }
  ...
  case NdbDictionary::Column::Tinyint: {
    ///< 8 bit. 1 byte signed integer, can be used in array
    return response->Append_i8(col_name, recAttr->int8_value());
  }
  case NdbDictionary::Column::Tinyunsigned: {
    ///< 8 bit. 1 byte unsigned integer, can be used in array
    return response->Append_iu8(col_name, recAttr->u_8_value());
  }
  case NdbDictionary::Column::Smallint: {
    ///< 16 bit. 2 byte signed integer, can be used in array
    return response->Append_i16(col_name, recAttr->short_value());
  }
  case NdbDictionary::Column::Smallunsigned: {
    ///< 16 bit. 2 byte unsigned integer, can be used in array
    return response->Append_iu16(col_name, recAttr->u_short_value());
  }
  case NdbDictionary::Column::Mediumint: {
    ///< 24 bit. 3 byte signed integer, can be used in array
    return response->Append_i24(col_name, recAttr->medium_value());
  }
  case NdbDictionary::Column::Mediumunsigned: {
    ///< 24 bit. 3 byte unsigned integer, can be used in array
    return response->Append_iu24(col_name, recAttr->u_medium_value());
  }
  case NdbDictionary::Column::Int: {
    ///< 32 bit. 4 byte signed integer, can be used in array
    return response->Append_i32(col_name, recAttr->int32_value());
  }
  case NdbDictionary::Column::Unsigned: {
    ///< 32 bit. 4 byte unsigned integer, can be used in array
    return response->Append_iu32(col_name, recAttr->u_32_value());
  }
  case NdbDictionary::Column::Bigint: {
    ///< 64 bit. 8 byte signed integer, can be used in array
    return response->Append_i64(col_name, recAttr->int64_value());
  }
  case NdbDictionary::Column::Bigunsigned: {
    ///< 64 Bit. 8 byte signed integer, can be used in array
    return response->Append_iu64(col_name, recAttr->u_64_value());
  }
  case NdbDictionary::Column::Float: {
    ///< 32-bit float. 4 bytes float, can be used in array
    return response->Append_f32(col_name, recAttr->float_value());
  }
  case NdbDictionary::Column::Double: {
    ///< 64-bit float. 8 byte float, can be used in array
    return response->Append_d64(col_name, recAttr->double_value());
  }
  case NdbDictionary::Column::Olddecimal: {
    ///< MySQL < 5.0 signed decimal,  Precision, Scale
    return RS_SERVER_ERROR(
  ...
  case NdbDictionary::Column::Decimalunsigned: {
    ...
    int precision = col->getPrecision();
    int scale = col->getScale();
    void *bin = (void*)recAttr->aRef();
    int binLen = recAttr->get_size_in_bytes();
    decimal_bin2str(bin,
                    binLen,
                    precision,
                    scale,
  ...
  case NdbDictionary::Column::Longvarchar: {
    ...
    Uint32 attrBytes;
    const char *dataStart = nullptr;
    if (unlikely(GetByteArray(recAttr, &dataStart, &attrBytes) != 0)) {
      return RS_CLIENT_ERROR(ERROR_019);
    } else {
      return response->Append_char(col_name,
                                   dataStart,
  ...
  case NdbDictionary::Column::Date: {
    ///< Precision down to 1 day(sizeof(Date) == 4 bytes )
    MYSQL_TIME lTime;
    my_unpack_date(&lTime, recAttr->aRef());
    char to[MAX_DATE_STRING_REP_LENGTH];
    my_date_to_str(lTime, to);
    return response->Append_string(col_name, std::string(to),
                                   RDRS_DATETIME_DATATYPE);
  ...
  case NdbDictionary::Column::Year: {
    ///< Year 1901-2155 (1 byte)
    Int32 year = (uint)(1900 + recAttr->aRef()[0]);
    return response->Append_i32(col_name, year);
  }
  case NdbDictionary::Column::Timestamp: {
    ///< Unix time
  ...
  case NdbDictionary::Column::Time2: {
    ///< 3 bytes + 0-3 fraction
    uint precision = col->getPrecision();
    const unsigned char* col_ptr = (const unsigned char*)recAttr->aRef();
    longlong numericTime = my_time_packed_from_binary(col_ptr, precision);
    MYSQL_TIME lTime;
    TIME_from_longlong_time_packed(&lTime, numericTime);
    char to[MAX_DATE_STRING_REP_LENGTH];
    my_TIME_to_str(lTime, to, precision);
  ...
  case NdbDictionary::Column::Datetime2: {
    ...
    uint precision = col->getPrecision();
    const unsigned char* col_ptr = (const unsigned char*)recAttr->aRef();
    longlong numericDate = my_datetime_packed_from_binary(col_ptr, precision);
    MYSQL_TIME lTime;
    TIME_from_longlong_datetime_packed(&lTime, numericDate);
    char to[MAX_DATE_STRING_REP_LENGTH];
    my_TIME_to_str(lTime, to, precision);
  ...
  case NdbDictionary::Column::Timestamp2: {
    ...
    my_timeval myTV{};
    const unsigned char* col_ptr = (const unsigned char*)recAttr->aRef();
    my_timestamp_from_binary(&myTV, col_ptr, precision);
    Int64 epochIn = myTV.m_tv_sec;
    time_t stdtime(epochIn);
    struct tm *time_info = gmtime(&stdtime);
    MYSQL_TIME lTime  = {};

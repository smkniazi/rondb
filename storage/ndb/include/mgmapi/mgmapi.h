 ...
 *  -# A non-constant pointer value.  A <var>NULL</var> value indicates an
 * error; otherwise, the return value must be freed by the programmer
 *  -# A constant pointer value, with a <var>NULL</var> value indicating an
 * error. The returned value should <em>not</em> be freed.
 *
 * Error conditions can be identified by using the appropriate
 * error-reporting functions ndb_mgm_get_latest_error() and
 * @ref ndb_mgm_error.
 *
 * Here is an example using the MGM API (without error handling for brevity's
 * sake).
 * @code
 ...
 *                         or <var>NULL</var> if no management handle could be
 * created.
 */
 ...
 */
const char *ndb_mgm_get_connectstring(NdbMgmHandle handle, char *buf,
                                      int buf_sz);
 ...
 */
int ndb_mgm_get_version(NdbMgmHandle handle, int *major, int *minor, int *build,
                        int len, char *str);
 ...
 */
struct ndb_mgm_node_state2 *ndb_mgm_get_node_status(
    struct ndb_mgm_cluster_state2 *cs, int i);
 ...
 */
int ndb_mgm_get_status_node_count(struct ndb_mgm_cluster_state2 *cs);
 ...
 */
int ndb_mgm_dump_state(NdbMgmHandle handle, int nodeId, const int *args,
                       int num_args, struct ndb_mgm_reply *reply);
 ...
 */
struct ndb_mgm_configuration *ndb_mgm_get_configuration_from_node(
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_stop2(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
                  int abort);
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_stop3(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_stop4(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_restart2(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_restart3(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
 ...
 * @return                Number of nodes stopped (-1 on error).
 */
int ndb_mgm_restart4(NdbMgmHandle handle, int no_of_nodes, const int *node_list,
   ...
   *
   * @param   handle        Management handle.
   * @param   nodeId        Node ID of node to get new hostname
   ...
   *
   * @param   handle        Management handle.
   * @param   nodeId        Node ID of node to be activated
   *
   * @return                0 for success, -1 on error
   *
   ...
   *
   * @param   handle        Management handle.
   * @param   nodeId        Node ID of node to be deactivated
   *
   * @return                0 for success, -1 on error
   *
 ...
 * @return                -1 on error.
 */
int ndb_mgm_set_clusterlog_loglevel(NdbMgmHandle handle, int nodeId,
                                    enum ndb_mgm_event_category category,
                                    int level, struct ndb_mgm_reply *reply);
 ...
 * @return                -1 on error.
 */
int ndb_mgm_set_loglevel_node(NdbMgmHandle handle, int nodeId,
                              enum ndb_mgm_event_category category, int level,
                              struct ndb_mgm_reply *reply);
#endif
 ...
 */
int ndb_logevent_get_next(const NdbLogEventHandle, struct ndb_logevent *dst,
 ...
 * @note Return value <=0 will leave dst untouched
 */
int ndb_logevent_get_next2(const NdbLogEventHandle, struct ndb_logevent *dst,
                           unsigned timeout_in_milliseconds);
...
int ndb_mgm_start_backup(NdbMgmHandle handle, int wait_completed,
                         unsigned int *backup_id, struct ndb_mgm_reply *reply);
 ...
 * @param   backup_id       Backup ID is returned from function.
 * @param   reply           Reply message.
 * @param   input_backupId  run as backupId and set next backup id to
 * input_backupId+1.
 * @return                  -1 on error.
 * @note                    backup_id will not be returned if
 *                          wait_completed == 0
 ...
 * @param   input_backupId        run as backupId and set next backup id to
 * input_backupId+1.
 * @param   backuppoint           Backup happen at start time(1) or complete
 * time(0).
...
int ndb_mgm_start_backup4(NdbMgmHandle handle, int wait_completed,
                          unsigned int *backup_id, struct ndb_mgm_reply *reply,
                          unsigned int input_backupId, unsigned int backuppoint,
...
int ndb_mgm_get_int_parameter(const ndb_mgm_configuration_iterator *, int param,
                              unsigned *value);
...
int ndb_mgm_create_nodegroup(NdbMgmHandle handle, int *nodes, int *ng,
                             struct ndb_mgm_reply *mgmreply);

int ndb_mgm_drop_nodegroup(NdbMgmHandle handle, int ng,
                           struct ndb_mgm_reply *mgmreply);
...
static inline int ndb_mgm_filter_clusterlog(NdbMgmHandle h,
                                            enum ndb_mgm_clusterlog_level s,
                                            int e, struct ndb_mgm_reply *r) {
  return ndb_mgm_set_clusterlog_severity_filter(
      h, (enum ndb_mgm_event_severity)s, e, r);
}
static inline const unsigned int *ndb_mgm_get_logfilter(NdbMgmHandle h) {
  return ndb_mgm_get_clusterlog_severity_filter_old(h);
}

static inline int ndb_mgm_set_loglevel_clusterlog(NdbMgmHandle h, int n,
...
static inline const unsigned int *ndb_mgm_get_loglevel_clusterlog(
    NdbMgmHandle h) {

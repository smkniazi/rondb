...
int UtilTransactions::clearTable(Ndb *pNdb, NdbScanOperation::ScanFlag flags,
                                 int records, int parallelism) {
  ...
  restart:
    ...
    while ((check = pOp->nextResult(true)) == 0) {
      ...
      if (check != -1) {
        ...
        if (check != -1) pTrans->getGCI(&m_util_latest_gci);
        pTrans->restart();
      }
...
int UtilTransactions::clearTable(Ndb *pNdb, int records, int parallelism) {
  return clearTable(pNdb, (NdbScanOperation::ScanFlag)0, records, parallelism);
}

int UtilTransactions::clearTable1(Ndb *pNdb, int records, int parallelism) {
  return clearTable(pNdb, (NdbScanOperation::ScanFlag)0, records, 1);
}

int UtilTransactions::clearTable2(Ndb *pNdb, int records, int parallelism) {
  return clearTable(pNdb, (NdbScanOperation::ScanFlag)0, records, parallelism);
}

int UtilTransactions::clearTable3(Ndb *pNdb, int records, int parallelism) {
  return clearTable(pNdb, (NdbScanOperation::ScanFlag)0, records, parallelism);
}

int UtilTransactions::copyTableData(Ndb *pNdb, const char *destName) {
...
int UtilTransactions::addRowToInsert(Ndb *pNdb, NdbConnection *pInsTrans,
                                     NDBT_ResultRow &row,
...
int UtilTransactions::scanReadRecords(Ndb *pNdb, int parallelism,
                                      NdbOperation::LockMode lm, int records,
                                      int noAttribs, int *attrib_list,
                                      ReadCallBackFn *fn) {
  int retryAttempt = 0;
  ...
  while (true) {
    ...
    for (int a = 0; a < noAttribs; a++) {
      if (attrib_list[a] < tab.getNoOfColumns()) {
        ...
        if ((row.attributeStore(attrib_list[a]) = pOp->getValue(
                 tab.getColumn(attrib_list[a])->getName())) == 0) {
          NDB_ERR(pTrans->getNdbError());
          closeTransaction(pNdb);
          return NDBT_FAILED;
        }
...
int UtilTransactions::selectCount(Ndb *pNdb, int parallelism, int *count_rows,
...
int UtilTransactions::verifyIndex(Ndb *pNdb, const char *indexName,
...
int UtilTransactions::verifyIndex(Ndb *pNdb,
                                  ...
                                  bool checkFromIndex, bool findNulls) {
  if (m_verbosity > 0) {
    ndbout << "|- Checking index " << targetIndex->getName()
           << " options (checkFromIndex " << checkFromIndex << " findNulls "
           << findNulls << ")" << endl;
  }
  if (targetIndex) {
    if (!checkFromIndex) {
      ...
      switch (targetIndex->getType()) {
        ...
        case NdbDictionary::Index::OrderedIndex:
          return verifyOrderedIndex(pNdb, NULL, targetIndex, 1, true,
                                    findNulls);
        default:
      ...
      switch (targetIndex->getType()) {
        case NdbDictionary::Index::UniqueHashIndex:
          ...
          return NDBT_OK;
        case NdbDictionary::Index::OrderedIndex:
          return verifyOrderedIndex(pNdb, targetIndex, NULL, 1, true,
                                    findNulls);
        default:
          ndbout << "Unknown index type" << endl;
          return NDBT_FAILED;
...
int UtilTransactions::verifyUniqueIndex(Ndb *pNdb,
                                        const NdbDictionary::Index *pIndex,
                                        int parallelism, bool transactional) {
  /**
  ...
  if (scanAndCompareUniqueIndex(pNdb, pIndex, parallelism, transactional) !=
      NDBT_OK) {
    return NDBT_FAILED;
  }
...
int UtilTransactions::scanAndCompareUniqueIndex(
    Ndb *pNdb, const NdbDictionary::Index *pIndex, int parallelism,
    bool transactional) {
  int retryAttempt = 0;
  const int retryMax = 100;
  int check;
  ...
  restart:
    ...
    while ((eof = pOp->nextResult()) == 0) {
      ...
      if (readRowFromTableAndIndex(pNdb, pTrans, pIndex, row) != NDBT_OK) {
        while ((eof = pOp->nextResult(false)) == 0)
          ;
        if (eof == 2) eof = pOp->nextResult(true);  // this should give -1
        if (eof == -1) {
          const NdbError err = pTrans->getNdbError();
...
int UtilTransactions::readRowFromTableAndIndex(
    Ndb *pNdb, NdbConnection *scanTrans, const NdbDictionary::Index *pIndex,
    NDBT_ResultRow &row) {
  ...
  while (true) {
    if (retryAttempt) ndbout_c("retryAttempt %d", retryAttempt);
    if (retryAttempt >= retryMax) {
      g_err << "ERROR: has retried this operation " << retryAttempt
            << " times, failing!, line: " << __LINE__ << endl;
      goto close_all;
...
close_all:
  if (pTrans1) pNdb->closeTransaction(pTrans1);

  return return_code;
}

int UtilTransactions::verifyOrderedIndex(
    Ndb *pNdb, const NdbDictionary::Index *sourceIndex,
    const NdbDictionary::Index *destIndex, int parallelism, bool transactional,
    bool findNulls) {
  int retryAttempt = 0;
  const int retryMax = 100;
  int check;
  ...
  while (true) {
    ...
    while (check == 0 && (eof = pOp->nextResult()) == 0) {
      ...
      if (!pk || pk->readTuple(NdbOperation::LM_CommittedRead)) goto error;
      if (equal(&tab, pk, scanRow) || get_values(pk, pkRow)) goto error;
      ...
      if (checkDestIndex) {
        ...
        if ((iop = pTrans->getNdbIndexScanOperation(destIndex->getName(),
                                                    tab.getName())) != NULL) {
          if (iop->readTuples(NdbScanOperation::LM_CommittedRead, parallelism))
            goto error;
          if (get_values(iop, indexRow)) goto error;
          if (equal(destIndex, iop, scanRow, true)) goto error;
        } else {
          goto error;
        }
      }
...
int UtilTransactions::verifyTableAndAllIndexes(Ndb *pNdb, bool findNulls,
                                               bool bidirectional, bool views,
                                               bool allSources) {
...
int UtilTransactions::verifyTableReplicas(Ndb *pNdb, bool allSources) {
...
int UtilTransactions::verifyTableReplicasPkCompareRow(
    Ndb *pNdb, Uint32 nodeId, const NDBT_ResultRow &scanRow) {
  NDBT_ResultRow pkRow(tab);
  ...
  if (nodeTrans->execute(Commit, AbortOnError) != 0) {
    ...
    NDB_ERR(err);
    nodeTrans->close();
    if (err.status == NdbError::TemporaryError) return NDBT_TEMPORARY;
    return NDBT_FAILED;
  }
...
int UtilTransactions::verifyTableReplicasScanAndCompareNodes(
    Ndb *pNdb, Uint32 sourceNodeId, Uint32 numDataNodes,
  ...
  while ((eof = pScan->nextResult()) == 0) {
    ...
    for (Uint32 n = 0; n < numDataNodes; n++) {
      const int result =
          verifyTableReplicasPkCompareRow(pNdb, dataNodes[n], scanRow);
      if (result == NDBT_OK) continue;
  ...
  if (mismatchRows) {
    g_err << "|- Checked " << rows << " rows with " << checks
          << " checks across " << numDataNodes << " data nodes." << endl;
    g_err << "  Found " << mismatchReplicas << " mismatches in " << mismatchRows
          << " rows" << endl;
    return NDBT_FAILED;
  }

  if (m_verbosity > 0) {
    ndbout << "|- Checked " << rows << " rows with " << checks
...
int UtilTransactions::verifyTableReplicasWithSource(Ndb *pNdb,
                                                    Uint32 sourceNodeId) {
  int retryAttempt = 0;
  const int retryMax = 100;
  ...
  if (m_verbosity > 0) {
    ndbout_c(
        "|- Checking replicas of table %s with source node %u from %u data "
        "nodes",
        tab.getName(), sourceNodeId, numDataNodes);
  }

  while (true) {
    const int result = verifyTableReplicasScanAndCompareNodes(
        pNdb, sourceNodeId, numDataNodes, dataNodes);
    closeTransaction(pNdb);
...
int UtilTransactions::verifyAllIndexes(Ndb *pNdb, bool findNulls,
                                       bool bidirectional, bool views) {
  NdbDictionary::Dictionary::List indexList;

  if (pNdb->getDictionary()->listIndexes(indexList, tab.getName()) != 0) {
    ndbout << " Failed to list indexes on table " << tab.getName() << " Error "
  ...
  for (unsigned i = 0; i < indexList.count; i++) {
    ...
    const NdbDictionary::Index *index =
        pNdb->getDictionary()->getIndex(indexName, tab);
    if (index == NULL) {
    ...
    if (verifyIndex(pNdb, index, false, /* Scan table, check index */
                    findNulls) != NDBT_OK) {
      return NDBT_FAILED;
    }
    ...
    if (bidirectional) {
      ...
      if (verifyIndex(pNdb, index, true, /* Scan table, check index */
                      findNulls) != NDBT_OK) {
        return NDBT_FAILED;
      }
    }
    ...
    if (views) {
       ...
       * aligned
       */
      if (verifyIndexViews(pNdb, index) != NDBT_OK) {
        return NDBT_FAILED;
      }
    }
  }
...
int UtilTransactions::verifyIndexViews(Ndb *pNdb,
                                       const NdbDictionary::Index *pIndex) {
  switch (pIndex->getType()) {
    case NdbDictionary::Index::UniqueHashIndex:
      ...
      return NDBT_OK;
    case NdbDictionary::Index::OrderedIndex:
      return verifyOrderedIndexViews(pNdb, pIndex);
    default:
      ndbout << "Unknown index type" << endl;
      break;
  }
 ...
 * nodes
 */
int UtilTransactions::verifyOrderedIndexViews(
    Ndb *pNdb, const NdbDictionary::Index *index) {
  Uint32 numDataNodes = 0;
  Uint32 dataNodes[256];
  {
    Ndb_cluster_connection *ncc = &pNdb->get_ndb_cluster_connection();
  ...
  for (Uint32 comparison = 0; comparison < numDataNodes - 1; comparison++) {
    if (verifyTwoOrderedIndexViews(pNdb, index, dataNodes[comparison],
 ...
 * the same
 */
int UtilTransactions::verifyTwoOrderedIndexViews(
    Ndb *pNdb, const NdbDictionary::Index *index, Uint32 node1, Uint32 node2) {
  int retryAttempt = 0;
  const int retryMax = 100;
  ...
  if (m_verbosity > 0) {
    ndbout << "|- Checking views of ordered index " << index->getName()
           << " on table " << tab.getName()
           << " from two data nodes : " << node1 << ", " << node2 << endl;
  }
  ...
  while (true) {
    ...
    if (defineOrderedScan(pNdb, index, node1, scan1Trans, scan1Op, scan1row) !=
        0) {
      return NDBT_FAILED;
    }

    if (defineOrderedScan(pNdb, index, node2, scan2Trans, scan2Op, scan2row) !=
        0) {
      scan1Trans->close();
      return NDBT_FAILED;
    }
    ...
    while (true) {
      ...
      if (eof1 == -1 || eof2 == -1) {
        /* Error */
        const NdbError err =
            (eof1 == -1) ? scan1Op->getNdbError() : scan2Op->getNdbError();
      ...
      if (eof1 || eof2) {
        ...
        g_err << "Error : Scan on node " << (eof1 ? node1 : node2)
      ...
      if (scan1row.c_str() != scan2row.c_str()) {
        g_err << "Error when comparing entries for index " << index->getName()
              << endl;
...
int UtilTransactions::defineOrderedScan(Ndb *pNdb,
                                        const NdbDictionary::Index *index,
                                        Uint32 nodeId,
  ...
  while (true) {
    ...
    NdbIndexScanOperation *op =
        trans->getNdbIndexScanOperation(index->getName(), tab.getName());
...
int UtilTransactions::get_values(NdbOperation *op, NDBT_ResultRow &dst) {
  for (int a = 0; a < tab.getNoOfColumns(); a++) {
...
int UtilTransactions::equal(const NdbDictionary::Index *pIndex,
                            NdbOperation *op, const NDBT_ResultRow &src,
                            bool skipNull) {
  for (Uint32 a = 0; a < pIndex->getNoOfColumns(); a++) {
    const NdbDictionary::Column *col = pIndex->getColumn(a);
    if (skipNull && src.attributeStore(col->getName())->isNULL()) {
      // ndbout_c("equal() exits before col %u", a);
      /* Have defined as many bounds as we can */
      return 0;
    }
    if (op->equal(col->getName(), src.attributeStore(col->getName())->aRef()) !=
        0) {
...
int UtilTransactions::equal(const NdbDictionary::Table *pTable,
                            NdbOperation *op, const NDBT_ResultRow &src) {
...
NdbScanOperation *UtilTransactions::getScanOperation(NdbConnection *pTrans) {
  return (NdbScanOperation *)getOperation(pTrans,
                                          NdbOperation::OpenScanRequest);
}

NdbOperation *UtilTransactions::getOperation(NdbConnection *pTrans,
    ...
    case NdbOperation::ReadExclusive:
      if (idx) {
        switch (idx->getType()) {
          ...
          case NdbDictionary::Index::OrderedIndex:
            return pTrans->getNdbIndexScanOperation(idx->getName(),
                                                    tab.getName());
          default:
            abort();
        }
...
int UtilTransactions::closeTransaction(Ndb *pNdb) {
  if (pTrans != NULL) {
...
int UtilTransactions::compare(Ndb *pNdb, const char *tab_name2, int flags) {
  NdbError err;
  ...
  if (tmp == 0) {
    g_err << "Unable to lookup table: " << tab_name2 << endl
          << pNdb->getDictionary()->getNdbError() << endl;
    return -1;
  }
  ...
  error:
    if (err.status == NdbError::TemporaryError) {
      ...
      NdbSleep_MilliSleep(50);
      closeTransaction(pNdb);
      if (cmp.getTransaction()) cmp.closeTransaction(pNdb);
...
void UtilTransactions::setVerbosity(Uint32 v) { m_verbosity = v; }

Uint32 UtilTransactions::getVerbosity() const { return m_verbosity; }

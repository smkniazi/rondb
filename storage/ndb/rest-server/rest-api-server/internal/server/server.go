...
func (rc *RouterConext) StartRouter() error {
	...
	httpListner, err := net.Listen("tcp", fmt.Sprintf("%s:%d", rc.RESTServerIP, rc.RESTServerPort))
	if err != nil {
		log.Fatalf("HTTP server returned. Error: %v", err)
	}

	grpcListner, err := net.Listen("tcp", fmt.Sprintf("%s:%d", rc.GRPCServerIP, rc.GRPCServerPort))
	if err != nil {
	...
	go func() { // Start REST Server
		if config.Configuration().Security.EnableTLS {
			...
			err = rc.HttpServer.ServeTLS(httpListner, config.Configuration().Security.CertificateFile,
				config.Configuration().Security.PrivateKeyFile)
		} else {
	...
	go func() { // Start GRPC Server
		...
		rc.GRPCServer.Serve(grpcListner)
	}()
